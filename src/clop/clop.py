    ...
    if torch.cuda.is_available():
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(
                    "nvidia-smi command failed - NVIDIA drivers may not be installed"
                )
...
EXCLUDED_BIOTYPES = frozenset(
    {
        ...
        "random_dna",
        "biological_region",
        "unknown_biotype_fasta_header",
        "unknown_species_fasta_header",
        "unknown_biotype_bed_ref",
        "unknown_bed_feature",
        "unknown_gff_species",
    }
)
    ...
    for part in attributes_str.split(";"):
        ...
        if "=" in part:
            ...
            attributes[part.strip()] = (
                ""  # Storing as empty string if no value
            )
    return attributes
...
class BaseTokenizer:
    ...
    def __init__(
        self, vocab: Optional[Dict[str, int]] = None, k: Optional[int] = None
    ):  # k for DNA kmer
        self.k = k
        ...
        for token, idx in _user_vocab.items():
            if idx == self.pad_token_id or idx == self.unk_token_id:
                tokens_to_reindex_clash_pad_unk[token] = (
                    idx  # Will be re-indexed
                )
            elif (
                idx in occupied_indices
            ):  # User token's index is already taken by another user token
                tokens_to_reindex_clash_other[token] = (
                    idx  # Will be re-indexed
                )
        ...
        for token in sorted(all_tokens_to_reindex.keys()):
            ...
            occupied_indices.add(
                next_available_idx
            )  # Mark new index as occupied
    ...
    def encode(
        self, text_or_sequence: str, max_length: Optional[int] = None
    ) -> Tuple[List[int], int]:
        ...
        encoded_ids = [
            self.vocab.get(token, self.unk_token_id) for token in tokens
        ]
    ...
    def save_vocab(self, filepath: str):
        try:
            ...
            logger.error(
                f"Failed to save tokenizer vocabulary to {filepath}: {e}"
            )
    ...
    def load_vocab(cls, filepath: str) -> "BaseTokenizer":
        try:
            ...
            if (
                loaded_vocab.get(PAD_TOKEN) != 0
                or loaded_vocab.get(UNK_TOKEN) != 1
            ):
                logger.warning(
                    f"Loaded vocab from {filepath} has non-standard PAD/UNK token indices. These will be enforced to 0 and 1 respectively during instantiation."
                )
            ...
            logger.info(
                f"Tokenizer vocabulary loaded from {filepath} for {cls.__name__}"
            )
            return instance
        except FileNotFoundError:
            logger.error(f"Tokenizer vocabulary file not found: {filepath}")
            raise
        except Exception as e:
            logger.error(
                f"Error loading/parsing tokenizer vocabulary {filepath}: {e}"
            )
            raise
...
class KmerDNATokenizer(BaseTokenizer):
    ...
    def __init__(
        self, k: int = DEFAULT_KMER_K, vocab: Optional[Dict[str, int]] = None
    ):
        if k <= 0:
            raise ValueError("k-mer size k must be positive.")
    ...
    def tokenize(self, sequence: str) -> List[str]:
        ...
        for i in range(len(sequence) - self.k + 1):
            ...
            if all(
                c in VALID_DNA_CHARS_SET for c in kmer
            ):  # Looser check, allow N in k-mers
                kmers.append(kmer)
...
class CharDNATokenizer(BaseTokenizer):
    ...
    def __init__(self, vocab: Optional[Dict[str, int]] = None):
        super().__init__(
            vocab=vocab if vocab else DEFAULT_CHAR_DNA_VOCAB.copy()
        )
...
def build_vocab_from_data(
    ...
    logger.info(
        f"Building {tokenizer_name} vocab from {len(texts)} texts with min_freq={min_freq}"
    ...
    for i, text_item in enumerate(texts):
        if (i + 1) % 100000 == 0:
            logger.info(
                f"Processed {i + 1}/{len(texts)} for vocab building..."
            )
        tokens = temp_tokenizer_instance.tokenize(text_item)
        token_counts.update(tokens)
    logger.info(
        f"Token counting finished in {time.time() - build_start_time:.2f}s."
    )
    ...
    current_idx_for_builder = (
        2  # Start from 2, as BaseTokenizer will handle 0 and 1 for PAD/UNK
    )

    for token, count in token_counts.most_common():
        if (
            count >= min_freq
        ):  # Don't add PAD/UNK here, BaseTokenizer handles them.
            if token != PAD_TOKEN and token != UNK_TOKEN:
                vocab_dict_for_builder[token] = current_idx_for_builder
                current_idx_for_builder += 1

    logger.info(
        f"Built vocab with {len(vocab_dict_for_builder) + 2} effective tokens for {tokenizer_name} (including PAD/UNK). Total time: {time.time() - build_start_time:.2f}s"
    )
...
class LSTMEncoderONNX(nn.Module):
    ...
    def __init__(self, original_encoder: LSTMEncoder):
        ...
        if not self.lstm.batch_first:
            raise ValueError(
                "LSTMEncoderONNX requires the LSTM to have batch_first=True."
            )
...
class DNAClipModel(nn.Module):
    def __init__(
        ...
        self.logit_scale = nn.Parameter(
            torch.ones([]) * np.log(1 / initial_temperature)
        )
        self.encoder_type = encoder_type

    def encode_dna(
        self, dna_tokens: torch.Tensor, dna_lengths: torch.Tensor
    ) -> torch.Tensor:
    ...
    def forward(
        ...
        logit_scale_clamped = torch.clamp(
            self.logit_scale, max=math.log(100.0)
        )
        logits_per_dna = (
            dna_embeds @ text_embeds.t()
        ) * logit_scale_clamped.exp()
...
class DNADataset(Dataset):
    def __init__(
        ...
        dna_tokenizer: BaseTokenizer,
        text_tokenizer: TextTokenizer,
        max_dna_len: Optional[int],
        ...
        self.max_dna_len = (
            max_dna_len
            if max_dna_len is not None and max_dna_len > 0
            else None
        )
        self.max_text_len = (
            max_text_len
        ...
        self.metadata = (
            metadata if metadata else [{} for _ in range(len(sequences))]
        )

        if not (len(sequences) == len(annotations) == len(self.metadata)):
            raise ValueError(
                "Sequences, annotations, and metadata must have the same length."
            )
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        dna_encoded_ids, dna_effective_len = self.dna_tokenizer.encode(
            seq, self.max_dna_len
        )
        text_encoded_ids, text_effective_len = self.text_tokenizer.encode(
            ann, self.max_text_len
        )
        ...
        if not dna_encoded_ids:
            dna_encoded_ids = [self.dna_tokenizer.pad_token_id] * (
                self.max_dna_len if self.max_dna_len else 1
            )
        if not text_encoded_ids:
            text_encoded_ids = [self.text_tokenizer.pad_token_id] * (
                self.max_text_len if self.max_text_len else 1
            )
        ...
        return {
            ...
            "raw_sequence": seq,
            "raw_sequence_len": len(seq),
            "raw_annotation": ann,
            "raw_annotation_len": len(ann.split()),  # Approx words
            "metadata": meta,
        }


def custom_collate_fn(
    batch: List[Dict[str, Any]], dna_pad_id: int, text_pad_id: int
) -> Dict[str, Any]:
    if not batch:
        return {}
    ...
    pad_ids = {
        "dna_tokens": float(dna_pad_id),
        "text_tokens": float(text_pad_id),
    }
    ...
    for key in keys_to_pad:
        ...
        if all(
            t.size(0) == tokens_list[0].size(0) for t in tokens_list
        ):  # Already same length (e.g. fixed max_len)
            collated_batch[key] = torch.stack(tokens_list)
        else:  # Needs padding
            collated_batch[key] = pad_sequence(
                tokens_list, batch_first=True, padding_value=pad_ids[key]
            )
        ...
        if key not in keys_to_pad:  # Handle other keys
            if isinstance(batch[0][key], torch.Tensor):
                ...
                collated_batch[key] = torch.tensor(
                    [item[key] for item in batch],
                    dtype=torch.long if "length" in key else torch.float,
                )
            else:  # Lists of strings (raw_sequence, raw_annotation, metadata)
                collated_batch[key] = [item[key] for item in batch]

    # Ensure lengths are tensors
    if "dna_lengths" in collated_batch and not isinstance(
        collated_batch["dna_lengths"], torch.Tensor
    ):
        collated_batch["dna_lengths"] = torch.tensor(
            collated_batch["dna_lengths"], dtype=torch.long
        )
    if "text_lengths" in collated_batch and not isinstance(
        collated_batch["text_lengths"], torch.Tensor
    ):
        collated_batch["text_lengths"] = torch.tensor(
            collated_batch["text_lengths"], dtype=torch.long
        )
...
def create_dataloader(
    ...
    collate_wrapper = lambda batch_items: custom_collate_fn(
        batch_items, dna_pad_id, text_pad_id
    )
    return DataLoader(
        dataset,
...
FASTA_HEADER_PATTERN = re.compile(
    r">([^\s|]+)(?:[|\s]*(.*))?"
)  # ID is group 1, rest is group 2


def parse_fasta(
    filepath: str, use_blacklist: bool = True
    ...
    try:
        with open_func(filepath, "rt", errors="ignore") as f:
            for line in f:
                ...
                if line.startswith(">"):
                    if current_seq_chunks and current_id:
                        ...
                        if (
                            use_blacklist
                            and current_meta.get("biotype", "unknown")
                            in EXCLUDED_BIOTYPES
                        ):
                    ...
                    species, biotype, description = parse_chromosample_header(
                        header
                    )
            ...
            if current_seq_chunks and current_id:
                ...
                if (
                    ...
                    data.append(
                        (current_id, "".join(current_seq_chunks), current_meta)
                    )
        ...
        logger.error(
            f"Error parsing FASTA file {filepath}: {e}", exc_info=True
        )
        raise

    if use_blacklist and skipped_blacklisted > 0:
        logger.info(
            f"Filtered out {skipped_blacklisted} sequences with blacklisted biotypes"
        )
...
def parse_bed(
    filepath: str, reference_fasta_path: str
    ...
    logger.info(
        f"Loading reference genome from {reference_fasta_path} for BED processing..."
    )
    try:
        parsed_fasta_ref = parse_fasta(reference_fasta_path)
        for header_id, seq, meta_dict in parsed_fasta_ref:
            chrom_name = header_id.split()[
                0
            ]  # Use first part of header as chrom name
        ...
        logger.info(
            f"Loaded {len(ref_genome_sequences)} chromosome/contig entries from reference FASTA for BED."
        )
    except Exception as e:
        logger.error(
            f"Error parsing reference FASTA {reference_fasta_path} for BED: {e}",
            exc_info=True,
        )
        raise
    ...
    try:
        ...
        with open_func(filepath, "rt", errors="ignore") as f:
            for i, line in enumerate(f):
                ...
                if len(fields) < 3:
                    logger.warning(
                        f"Skipping malformed BED line {i + 1} (fields<3): {line}"
                    )
                    continue
                ...
                name = (
                    ...
                    else f"region_{chrom}_{start_str}_{end_str}"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Skipping BED line {i + 1} (invalid coords): {line}"
                    )
                    continue
                ...
                resolved_chrom = next(
                    (
                        ...
                        + ([chrom[3:]] if chrom.startswith("chr") else [])
                        if c in ref_genome_sequences
                    ),
                    None,
                )
                ...
                if not resolved_chrom:
                    ...
                    if (
                        skipped_lines_chrom <= 10
                        or skipped_lines_chrom % 1000 == 0
                    ):  # Log first few and then periodically
                        logger.warning(
                            f"Chromosome '{chrom}' (BED line {i + 1}) not found in reference genome. Skipping. (Total such skips: {skipped_lines_chrom})"
                        )
                    continue
                ...
                if not (0 <= start < end <= len(chrom_seq)):
                    ...
                    if (
                        skipped_lines_coord <= 10
                        or skipped_lines_coord % 1000 == 0
                    ):
                        logger.warning(
                            f"Skipping BED line {i + 1} (invalid range for chr '{resolved_chrom}'): {start}-{end}, ref len {len(chrom_seq)}. (Total such skips: {skipped_lines_coord})"
                        )
                    continue
                ...
                meta["species"] = ref_genome_metadata.get(
                    resolved_chrom, {}
                ).get("species", "unknown_species_bed_ref")

                attributes_str = ""
                if (
                    len(fields) > 8 and fields[8] and fields[8] != "."
                ):  # Optional 9th field (attributes)
                    attributes_str = fields[8]
                elif (
                    len(fields) > 3
                    and fields[3]
                    and fields[3] != "."
                    and any(c in fields[3] for c in "=;")
                ):  # Check if name field contains attributes
                    attributes_str = fields[
                        3
                    ]  # Use name field if it looks like attributes

                parsed_desc_meta = {}
                if attributes_str:
                    parsed_desc_meta = parse_gff_attributes(
                        attributes_str
                    )  # Re-use GFF attribute parser

                meta["biotype"] = parsed_desc_meta.get(
                    "biotype",
                    parsed_desc_meta.get(
                        "gene_biotype",
                        ref_genome_metadata.get(resolved_chrom, {}).get(
                            "biotype", "unknown_bed_feature"
                        ),
                    ),
                )
                meta.update(
                    parsed_desc_meta
                )  # Add all parsed attributes to meta
                meta["raw_bed_fields"] = (
                ...
                annotation_text_parts = [
                    meta["biotype"],
                    name,
                    f"from {resolved_chrom}:{start}-{end}",
                ]
                if "description" in meta:
                    annotation_text_parts.append(meta["description"])
                elif meta["raw_bed_fields"] and meta["raw_bed_fields"] != name:
                    annotation_text_parts.append(
                        f"Attributes: {meta['raw_bed_fields']}"
                    )

                annotation_text = ", ".join(
                    filter(None, annotation_text_parts)
                ).strip()
        ...
        logger.error(f"BED file not found: {filepath}")
        raise
    except Exception as e:
        logger.error(f"Error parsing BED file {filepath}: {e}", exc_info=True)
        raise

    if skipped_lines_chrom > 0:
        logger.warning(
            f"Total BED lines skipped due to chromosome not in reference: {skipped_lines_chrom}"
        )
    if skipped_lines_coord > 0:
        logger.warning(
            f"Total BED lines skipped due to invalid coordinates for reference: {skipped_lines_coord}"
        )
...
def parse_gff3(
    filepath: str, reference_fasta_path: str, target_feature_types: List[str]
) -> List[Tuple[str, str, Dict[str, Any], str]]:
    """
    ...
    ref_genome_metadata = {}
    logger.info(
        f"Loading reference genome from {reference_fasta_path} for GFF3 processing..."
    )
    try:
        parsed_fasta_ref = parse_fasta(reference_fasta_path)
        for header_id, seq, meta_dict in parsed_fasta_ref:
            ...
            if (
                chrom_name.startswith("chr")
                and chrom_name[3:] not in ref_genome_sequences
            ):
                ref_genome_sequences[chrom_name[3:]] = seq
                ref_genome_metadata[chrom_name[3:]] = meta_dict
            elif (
                not chrom_name.startswith("chr")
                and f"chr{chrom_name}" not in ref_genome_sequences
            ):
                ref_genome_sequences[f"chr{chrom_name}"] = seq
                ref_genome_metadata[f"chr{chrom_name}"] = meta_dict

        logger.info(
            f"Loaded {len(ref_genome_sequences)} chromosome/contig entries from reference FASTA for GFF3."
        )
    except Exception as e:
        logger.error(
            f"Error parsing reference FASTA {reference_fasta_path} for GFF3: {e}",
            exc_info=True,
        )
        raise
    ...
    open_func = (
        gzip.open
        if filepath.endswith((".gz", ".gff3.gz", ".gff.gz"))
        else open
    )
    try:
        with open_func(filepath, "rt", errors="ignore") as f:
            for i, line in enumerate(f):
                ...
                if len(fields) != 9:
                    logger.warning(
                        f"Skipping malformed GFF3 line {i + 1} (fields != 9): {line}"
                    )
                    continue
                ...
                if (
                    ...
                    and feat_type not in target_feature_types
                ):
                ...
                try:
                    start = (
                        int(start_str) - 1
                    )  # GFF is 1-based, Python is 0-based start
                    end = int(
                        end_str
                    )  # GFF is 1-based, Python is 0-based exclusive end
                except ValueError:
                    logger.warning(
                        f"Skipping GFF3 line {i + 1} (invalid coords {start_str}-{end_str}): {line}"
                    )
                    continue

                resolved_chrom = chrom
                if (
                    chrom not in ref_genome_sequences
                ):  # Try common variations like 'chr1' vs '1'
                    alt_chrom = (
                        f"chr{chrom}"
                    ...
                    if alt_chrom in ref_genome_sequences:
                        ...
                        if (
                            ...
                            logger.warning(
                                f"Chromosome '{chrom}' (GFF3 line {i + 1}) not found in reference genome (tried '{alt_chrom}'). Skipping. (Total such skips: {skipped_lines_chrom})"
                            )
                        continue
                ...
                if not (0 <= start < end <= len(chrom_seq)):
                    ...
                    if (
                        ...
                        logger.warning(
                            f"Skipping GFF3 line {i + 1} (invalid range for chr '{resolved_chrom}'): {start + 1}-{end}, ref len {len(chrom_seq)}. (Total such skips: {skipped_lines_coord})"
                        )
                    continue
                ...
                gff_id = attributes.get(
                    "ID",
                    attributes.get(
                        "Name",
                        f"{feat_type}_{resolved_chrom}_{start + 1}_{end}",
                    ),
                )
                ...
                biotype = attributes.get(
                    ...
                    attributes.get(
                        "gene_biotype",
                        attributes.get("transcript_biotype", feat_type),
                    ),
                )

                # Species from reference FASTA metadata if available
                species = ref_genome_metadata.get(resolved_chrom, {}).get(
                    "species", "unknown_gff_species"
                )
                ...
                meta = {
                    ...
                    "chr": resolved_chrom,
                    "start": start + 1,
                    "end": end,  # Store original 1-based for meta
                    "strand": strand,
                    "source_format": "gff3",
                    "gff_feature_type": feat_type,
                    "biotype": biotype,
                    "species": species,
                    "gff_source_column": source,
                    "gff_score": score,
                    "gff_phase": phase,
                ...
                if "Name" in attributes:
                    ann_parts.append(attributes["Name"])
                elif meta["id"] != meta["biotype"]:
                    ann_parts.append(
                        meta["id"]
                    )  # Add ID if different from biotype and no Name

                ann_parts.append(
                    f"from {species} chromosome {resolved_chrom}:{meta['start']}-{meta['end']}({strand})"
                )
                ...
                if "product" in attributes:
                    ann_parts.append(f"Product: {attributes['product']}")
                if "gene" in attributes:
                    ann_parts.append(f"Gene: {attributes['gene']}")
                ...
                for attr_key in other_attrs_to_include:
                    if attr_key in attributes and attributes[attr_key]:
                        ann_parts.append(
                            f"{attr_key.replace('_', ' ').title()}: {attributes[attr_key]}"
                        )
                ...
                if processed_count % 10000 == 0:
                    logger.info(
                        f"Processed {processed_count} target GFF3 features..."
                    )

    except FileNotFoundError:
        logger.error(f"GFF3 file not found: {filepath}")
        raise
    except Exception as e:
        logger.error(f"Error parsing GFF3 file {filepath}: {e}", exc_info=True)
        raise

    logger.info(f"Finished parsing GFF3 file: {filepath}")
    logger.info(
        f"Total GFF3 lines processed for target types: {processed_count}"
    )
    if skipped_lines_target_type > 0:
        logger.info(
            f"GFF3 lines skipped because feature type not in target list: {skipped_lines_target_type}"
        )
    if skipped_lines_chrom > 0:
        logger.warning(
            f"Total GFF3 lines skipped due to chromosome not in reference: {skipped_lines_chrom}"
        )
    if skipped_lines_coord > 0:
        logger.warning(
            f"Total GFF3 lines skipped due to invalid coordinates for reference: {skipped_lines_coord}"
        )
    ...
    if len(gff_feature_type_counts) > 10:
        logger.info(
            f"  ... and {len(gff_feature_type_counts) - 10} other feature types."
        )
    ...
    dataset_summary_temp_storage["gff_feature_type_counts_raw"] = dict(
        gff_feature_type_counts
    )
...
def contrastive_loss(
    logits_per_source: torch.Tensor,
    logits_per_target: torch.Tensor,
    device: torch.device,
) -> torch.Tensor:
...
def train_epoch(
    ...
    if num_total_batches == 0:
        return 0.0, model.logit_scale.exp().item(), 0.0
    ...
    for i, batch in enumerate(dataloader):
        ...
        dna_lengths = batch[
            "dna_lengths"
        ]  # Stays on CPU for pack_padded_sequence
        ...
        with autocast_ctx:
            logits_dna, logits_text, temp = model(
                dna_tokens, dna_lengths, text_tokens, text_lengths
            )
            loss = contrastive_loss(logits_dna, logits_text, device)

        if torch.isnan(loss) or torch.isinf(loss):
            logger.warning(
                f"NaN/Inf loss encountered at Epoch {epoch_num + 1}, Batch {i + 1}. Skipping update for this batch."
            )
        else:
            ...
            if not (
                np.isnan(current_grad_norm) or np.isinf(current_grad_norm)
            ):
                total_grad_norm += current_grad_norm

        if (i + 1) % (
            max(1, num_total_batches // 10)
        ) == 0 or i == num_total_batches - 1:
            current_loss_val = (
                loss.item()
            ...
            logger.info(
                f"Epoch [{epoch_num + 1}/{num_epochs}], Batch [{i + 1}/{num_total_batches}], Loss: {current_loss_val:.4f}, Temp_eff: {temp.item():.4f}, GradNorm: {current_grad_norm:.4f}"
            )

    epoch_duration = time.time() - epoch_start_time
    logger.info(
        f"Train epoch {epoch_num + 1} completed in {epoch_duration:.2f}s."
    )
    if batches_processed == 0:
        return 0.0, model.logit_scale.exp().item(), 0.0
    return (
        total_loss / batches_processed,
        total_temp_val / batches_processed,
        total_grad_norm / batches_processed,
    )
...
def evaluate(
    ...
    epoch_str = (
        f"epoch {epoch + 1}" if epoch is not None else "Final Evaluation"
    )
    ...
    autocast_enabled_for_eval = (
        device.type == "cuda" and torch.is_autocast_enabled()
    )  # Check if it was enabled outside
    ...
    for i, batch in enumerate(dataloader):
        ...
        autocast_ctx_eval = (
            ...
            if autocast_enabled_for_eval
            else contextlib.nullcontext()
        )

        with (
            autocast_ctx_eval
        ):  # Use AMP for eval if it was used in training for consistency
            logits_dna, logits_text, _ = model(
                dna_tokens, dna_lengths, text_tokens, text_lengths
            )
            loss = contrastive_loss(logits_dna, logits_text, device)
        ...
        if not (torch.isnan(loss) or torch.isinf(loss)):
            ...
            logger.warning(
                f"NaN/Inf loss encountered in evaluation batch {i + 1} for {epoch_str}."
            )

        all_dna_embeds_list.append(
            dna_embeds.cpu().numpy(force=True)
        )  # force=True if from autocast non-blocking
        ...
        if isinstance(current_batch_metadata, dict) and not any(
            isinstance(v, list) for v in current_batch_metadata.values()
        ):
            ...
            try:
                ...
                for k_meta, v_meta_list in current_batch_metadata.items():
                    if (
                        isinstance(v_meta_list, list)
                        and len(v_meta_list) == num_items_in_batch
                    ):
                        for item_idx in range(num_items_in_batch):
                            reconstructed_meta[item_idx][k_meta] = v_meta_list[
                                item_idx
                            ]
                current_batch_metadata = reconstructed_meta
            except Exception as e_meta_recon:
                logger.error(
                    f"Error reconstructing metadata in evaluation: {e_meta_recon}. Metadata might be incomplete."
                )
                current_batch_metadata = [
                    {} for _ in range(dna_tokens.size(0))
                ]  # Fallback

        all_biotypes_eval.extend(
            [m.get("biotype", "unknown") for m in current_batch_metadata]
        )
        all_species_eval.extend(
            [m.get("species", "unknown") for m in current_batch_metadata]
        )

        if (
            is_final_eval
        ):  # Only collect raw texts for final detailed report to save memory
            raw_seqs_eval.extend(batch["raw_sequence"])
            raw_anns_eval.extend(batch["raw_annotation"])

        if (i + 1) % (
            max(1, num_total_batches // 5)
        ) == 0 or i == num_total_batches - 1:
            logger.info(
                f"Evaluation batch [{i + 1}/{num_total_batches}] for {epoch_str} processed."
            )
    ...
    current_metrics.update(
        {"mrr_dna_to_text": mrr_d2t, "mrr_text_to_dna": mrr_t2d}
    )
    metrics_agg["mrr_dna_to_text"].append(mrr_d2t)
    metrics_agg["mrr_text_to_dna"].append(mrr_t2d)
    logger.info(
        f"MRR DNA->Text: {mrr_d2t:.3f}, Text->DNA: {mrr_t2d:.3f} for {epoch_str}"
    )
    ...
    if num_samples_eval > 0:
        ...
        valid_recall_ks = (
            valid_recall_ks
            if valid_recall_ks
            else ([1] if num_samples_eval >= 1 else [])
        )

        top_k_indices_dna = np.argsort(-similarity_matrix, axis=1)
        for k_val in valid_recall_ks:
            recalls_dna_to_text[f"dna_to_text_R@{k_val}"] = np.mean(
                [
                    i in top_k_indices_dna[i, :k_val]
                    for i in range(num_samples_eval)
                ]
            )

        top_k_indices_text = np.argsort(
            -similarity_matrix.T, axis=1
        )  # Transpose for text-to-dna
        for k_val in valid_recall_ks:
            recalls_text_to_dna[f"text_to_dna_R@{k_val}"] = np.mean(
                [
                    i in top_k_indices_text[i, :k_val]
                    for i in range(num_samples_eval)
                ]
            )

    for k_met, v_met in recalls_dna_to_text.items():
        metrics_agg[k_met].append(v_met)
    for k_met, v_met in recalls_text_to_dna.items():
        metrics_agg[k_met].append(v_met)
    current_metrics.update(recalls_dna_to_text)
    current_metrics.update(recalls_text_to_dna)
    logger.info(
        f"Retrieval Recalls ({epoch_str}): {{**recalls_dna_to_text, **recalls_text_to_dna}}"
    )

    # k-NN Classification on Biotypes (if enough diverse data)
    unique_biotypes_for_knn = sorted(list(set(all_biotypes_eval)))
    if len(unique_biotypes_for_knn) >= 2 and all_dna_embeds.shape[0] > len(
        unique_biotypes_for_knn
    ):
        try:
            ...
            valid_indices_knn = [
                i
                for i, bio in enumerate(all_biotypes_eval)
                if biotype_counts_for_knn[bio] >= 2
            ]  # Need at least 2 for split

            if len(valid_indices_knn) >= max(
                10, 2 * len(unique_biotypes_for_knn) + 2
            ):  # Heuristic: need enough data overall
                knn_X_embeds = all_dna_embeds[valid_indices_knn]
                knn_Y_labels = [
                    all_biotypes_eval[i] for i in valid_indices_knn
                ]
                ...
                if (
                    all(c >= 2 for c in label_counts_for_split.values())
                    and len(label_counts_for_split) >= 2
                ):
                    X_train_knn, X_test_knn, y_train_knn, y_test_knn = (
                        train_test_split(
                            ...
                            knn_Y_labels,
                            test_size=0.3,
                            stratify=knn_Y_labels,
                            random_state=42,
                        )
                    )
                    if (
                        len(X_train_knn) > 0
                        and len(X_test_knn) > 0
                        and len(set(y_train_knn)) > 1
                    ):  # Ensure train/test are usable
                        n_neighbors_knn = min(
                            5,
                            len(X_train_knn),
                            min(Counter(y_train_knn).values()),
                        )  # Adapt k
                        n_neighbors_knn = max(
                            1, n_neighbors_knn
                        ...
                        ).fit(X_train_knn, y_train_knn)
                        y_pred_knn = knn_classifier.predict(X_test_knn)

                        report_labels = sorted(
                            list(set(y_test_knn) | set(y_pred_knn))
                        )  # All labels in test or pred

                        clf_report_dict = sk_classification_report(
                            y_test_knn,
                            y_pred_knn,
                            output_dict=True,
                            zero_division=0,
                        ...
                        if "accuracy" not in clf_report_dict:
                            clf_report_dict["accuracy"] = accuracy_score(
                                y_test_knn, y_pred_knn
                            )  # Ensure accuracy is there

                        current_metrics.update(
                            {
                                "knn_biotype_accuracy": clf_report_dict[
                                    "accuracy"
                                ],
                                "knn_biotype_f1_macro": clf_report_dict[
                                    "macro avg"
                                ]["f1-score"],
                            }
                        )
                        metrics_agg["knn_biotype_accuracy"].append(
                            current_metrics["knn_biotype_accuracy"]
                        )
                        metrics_agg["knn_biotype_f1_macro"].append(
                            current_metrics["knn_biotype_f1_macro"]
                        )
                        metrics_agg[
                            "knn_classification_reports_biotype"
                        ].append(clf_report_dict)
                        logger.info(
                            f"k-NN Biotype Classification ({epoch_str}): Accuracy {current_metrics['knn_biotype_accuracy']:.3f}, F1-macro {current_metrics['knn_biotype_f1_macro']:.3f}"
                        )
                        ...
                        plot_cm_condition = (
                            ...
                            or (
                                epoch is not None
                                and metrics_agg.get("best_epoch_idx", -1)
                                == epoch
                            )
                            or (
                                epoch is not None
                                and epoch
                                == metrics_agg.get("num_epochs_trained", 0) - 1
                            )
                        )

                        if plot_cm_condition:
                            cm = confusion_matrix(
                                y_test_knn, y_pred_knn, labels=report_labels
                            )
                            plt.figure(
                                figsize=(
                                    max(8, len(report_labels) * 0.6),
                                    max(6, len(report_labels) * 0.5),
                                )
                            )
                            sns.heatmap(
                                cm,
                            ...
                            plt.title(
                                f"k-NN Biotype Confusion Matrix ({epoch_tag_cm.replace('_', ' ').title()})"
                            )
                            plt.tight_layout()
                            ...
                            try:
                                ...
                                plt.close()
                                if "plot_paths" not in metrics_agg:
                                    metrics_agg["plot_paths"] = defaultdict(
                                        list
                                    )
                                # Avoid duplicate filenames if called multiple times for same epoch (e.g. final eval = best epoch)
                                if (
                                    not metrics_agg["plot_paths"][
                                        "knn_biotype_cm"
                                    ]
                                    or metrics_agg["plot_paths"][
                                        "knn_biotype_cm"
                                    ][-1]
                                    != cm_filename
                                ):
                                    metrics_agg["plot_paths"][
                                        "knn_biotype_cm"
                                    ].append(cm_filename)
                            except Exception as e_cm_save:
                                logger.error(
                                    f"Failed to save k-NN CM plot {cm_filepath}: {e_cm_save}"
                                )
                                plt.close()
                else:
                    logger.info(
                        f"Not enough data or classes with sufficient samples for stratified k-NN Biotype Classification for {epoch_str}."
                    )
            else:
                logger.info(
                    f"Not enough valid samples for k-NN Biotype Classification for {epoch_str}."
                )
        except ImportError:
            logger.warning(
                "scikit-learn (for KNeighborsClassifier) not available. Skipping k-NN Biotype Classification."
            )
        except Exception as e_knn:
            logger.error(
                f"Error during k-NN Biotype Classification for {epoch_str}: {e_knn}",
                exc_info=True,
            )

    # Clustering Quality Metrics (Silhouette, Davies-Bouldin)
    for label_type_cluster, labels_list_cluster in [
        ("biotype", all_biotypes_eval),
        ("species", all_species_eval),
    ]:
        unique_labels_for_clustering = sorted(list(set(labels_list_cluster)))
        if len(unique_labels_for_clustering) >= 2 and all_dna_embeds.shape[
            0
        ] > len(
            unique_labels_for_clustering
        ):  # Need at least 2 clusters and more samples than clusters
            try:
                silhouette = silhouette_score(
                    all_dna_embeds, labels_list_cluster
                )
                davies_bouldin = davies_bouldin_score(
                    all_dna_embeds, labels_list_cluster
                )
                current_metrics[f"silhouette_score_{label_type_cluster}"] = (
                    silhouette
                )
                current_metrics[
                    f"davies_bouldin_index_{label_type_cluster}"
                ] = davies_bouldin
                metrics_agg[f"silhouette_score_{label_type_cluster}"].append(
                    silhouette
                )
                metrics_agg[
                    f"davies_bouldin_index_{label_type_cluster}"
                ].append(davies_bouldin)
                logger.info(
                    f"Silhouette Score ({label_type_cluster}, {epoch_str}): {silhouette:.3f}"
                )
                logger.info(
                    f"Davies-Bouldin Index ({label_type_cluster}, {epoch_str}): {davies_bouldin:.3f}"
                )
            except ValueError as e_cluster_metric:  # Typically "Number of labels is N but should be between 2 and n_samples - 1"
                logger.warning(
                    f"Could not compute clustering metrics for {label_type_cluster} ({epoch_str}): {e_cluster_metric}"
                )
            except Exception as e_gen_cluster:
                logger.error(
                    f"Unexpected error computing clustering metrics for {label_type_cluster} ({epoch_str}): {e_gen_cluster}",
                    exc_info=True,
                )
        else:
            logger.info(
                f"Not enough distinct labels or samples for clustering metrics on {label_type_cluster} for {epoch_str}."
            )

    eval_duration = time.time() - eval_start_time
    logger.info(
        f"Evaluation for {epoch_str} complete in {eval_duration:.2f}s."
    )
    return (
        current_metrics,
        all_dna_embeds,
...
def plot_embeddings(
    ...
    if n_samples < 2:
        logger.warning(
            f"Not enough samples ({n_samples}) for embedding plot: {title}. Skipping."
        )
        return False
    ...
    if n_samples < 5 and method in ["tsne", "umap"]:
        logger.info(
            f"Number of samples ({n_samples}) is too small for {method.upper()}, defaulting to PCA for plot: {title}."
        )
        method = "pca"

    if method == "umap" and not umap:
        logger.info(
            f"UMAP module not installed, defaulting to PCA for plot: {title}."
        )
        method = "pca"

    logger.info(
        f"Generating embedding plot: {title} using {method.upper()}..."
    )
    embeddings_2d = None
    try:
        if embeddings.shape[1] < 2:  # Input embeddings are 1D
            if embeddings.shape[1] == 1:
                embeddings_2d = np.hstack(
                    (embeddings, np.zeros_like(embeddings))
                )  # Make it 2D by adding zeros
            else:  # embeddings.shape[1] == 0, should not happen with valid model
                logger.warning(
                    f"Embeddings have dimension {embeddings.shape[1]} for {title}, cannot plot."
                )
                return False
        elif method == "pca":
            ...
            if (
                pca_n_components < 2 and pca_n_components == 1
            ):  # Can only do 1D PCA
                pca_result = PCA(
                    n_components=1, random_state=42, svd_solver="full"
                ).fit_transform(embeddings)
                embeddings_2d = np.hstack(
                    (pca_result, np.zeros_like(pca_result))
                )  # Make 2D
            elif pca_n_components < 1:
                logger.warning(
                    f"Cannot perform PCA with {pca_n_components} components for {title}. Skipping plot."
                )
                return False
            else:  # Standard 2D PCA
                embeddings_2d = PCA(
                    n_components=2, random_state=42, svd_solver="full"
                ).fit_transform(embeddings)
        elif method == "tsne":
            # Adjust perplexity and n_iter based on sample size
            tsne_perplexity = min(
                30.0, float(max(1, n_samples - 2))
            )  # Perplexity must be less than n_samples - 1
            tsne_n_iter = max(
                250, n_samples * 4
            )  # More iterations for larger N
            tsne_init_method = (
                "pca"
                if n_samples > 2 and embeddings.shape[1] >= 2
                else "random"
            )  # PCA init needs >2 components possible
            embeddings_2d = TSNE(
                n_components=2,
                random_state=42,
                perplexity=tsne_perplexity,
                n_iter=tsne_n_iter,
                init=tsne_init_method,
                learning_rate="auto",
            ).fit_transform(embeddings)
        elif (
            method == "umap" and umap
        ):  # umap is guaranteed to be available here by prior check
            umap_n_neighbors = min(
                15, max(2, n_samples - 1 if n_samples > 1 else 2)
            )  # n_neighbors must be less than n_samples
            umap_min_dist = (
                0.1 if n_samples > 15 else 0.5
            )  # Adjust min_dist for small N
        ...
        if (
            embeddings_2d is None
        ):  # Fallback if a method was chosen but failed for some reason (e.g. UMAP not installed and logic error)
            pca_n_components_fb = min(
                2, embeddings.shape[0], embeddings.shape[1]
            )
            if pca_n_components_fb < 1:
                logger.error(
                    "Fallback PCA failed due to insufficient components."
                )
                return False
            logger.warning(
                f"Dimensionality reduction failed for {title}, attempting fallback PCA."
            )
            embeddings_2d = PCA(
                n_components=pca_n_components_fb,
                random_state=42,
                svd_solver="full",
            ).fit_transform(embeddings)
            if pca_n_components_fb == 1:
                embeddings_2d = np.hstack(
                    (embeddings_2d, np.zeros_like(embeddings_2d))
                )

    except Exception as e_reduce:
        logger.error(
            f"Dimensionality reduction ({method.upper()}) failed for {title}: {e_reduce}. Skipping plot.",
            exc_info=True,
        )
        return False
    ...
    if embeddings_2d is None or embeddings_2d.shape[1] < 2:
        logger.error(
            f"Dimensionality reduction resulted in < 2 dimensions for {title}. Skipping plot."
        )
        return False

    plt.figure(figsize=(12, 10))
    unique_plot_labels = sorted(
        list(set(l for l in labels if l and str(l).strip()))
    )  # Filter out None or empty string labels

    if (
        not unique_plot_labels or len(unique_plot_labels) == 1
    ):  # Only one type of label or no labels
        label_name = (
            unique_plot_labels[0] if unique_plot_labels else "Data Points"
        )
        plt.scatter(
            embeddings_2d[:, 0],
            embeddings_2d[:, 1],
        ...
        try:
            ...
            ValueError
        ):  # If len(unique_plot_labels) is too small for gist_rainbow
            cmap = plt.get_cmap("viridis", len(unique_plot_labels))

        for i, label_val in enumerate(unique_plot_labels):
            indices = [
                j
                for j, current_lab in enumerate(labels)
                if current_lab == label_val
            ]
            if indices:
                plt.scatter(
        ...
        num_legend_cols = min(
            5,
            (len(unique_plot_labels) // 10) + 1
            if len(unique_plot_labels) > 1
            else 1,
        )  # Adapt columns
        plt.legend(
            loc="upper center",
    ...
    try:
        ...
        logger.error(
            f"Failed to save embedding plot {filepath}: {e_save}",
            exc_info=True,
        )
        plt.close()
        return False
...
def plot_metric_trends(metrics_history: Dict, report_dir: str):
    ...
    if num_epochs_trained == 0:
        logger.info(
            "No epochs trained or metrics history empty, skipping metric trend plots."
        )
        return {}
    ...
    metric_groups_to_plot = {
        ...
        "temperature": (
            ["temperature"],
            "Learned Temperature (1 / Logit Scale)",
            "Temperature",
        ),
        "gradient_norm": (
            ["avg_grad_norm"],
        ...
        "retrieval_metrics": (
            [
                m_key
                for m_key in metrics_history
                if "R@" in m_key or "mrr_" in m_key
            ],
        ...
        "knn_biotype_metrics": (
            [
                m_key
                for m_key in metrics_history
                if "knn_biotype_" in m_key and "report" not in m_key
            ],
            "k-NN Biotype Classification Metrics (Validation Set)",
            "Score",
        ),
        "clustering_metrics": (
            [
                m_key
                for m_key in metrics_history
                if "silhouette_" in m_key or "davies_bouldin_" in m_key
            ],
            "Clustering Quality Metrics (Validation Set)",
            "Score",
        ),
    }

    for plot_filename_key, (
        metric_keys_in_group,
        plot_title,
        y_axis_label,
    ) in metric_groups_to_plot.items():
        plt.figure(figsize=(10, 6))
        data_plotted_flag = False

        actual_metrics_in_group = [
            mk
            for mk in metric_keys_in_group
            if mk in metrics_history and len(metrics_history[mk]) > 0
        ]

        for metric_name in actual_metrics_in_group:
            metric_values = metrics_history[metric_name][
                :num_epochs_trained
            ]  # Ensure correct length
            if metric_values:  # Check if list is not empty
                current_epochs_axis = epochs_axis[
                    : len(metric_values)
                ]  # Align x-axis with available data points
                plt.plot(
                    current_epochs_axis,
                    metric_values,
                    marker="o",
        ...
        if data_plotted_flag:
            ...
            plot_filepath = os.path.join(
                report_dir, f"{plot_filename_key}.png"
            )
            try:
                plt.savefig(plot_filepath)
                plt.close()
                generated_plot_paths[plot_filename_key] = os.path.basename(
                    plot_filepath
                )
            except Exception as e_trend_save:
                logger.error(
                    f"Failed to save metric trend plot {plot_filepath}: {e_trend_save}",
                    exc_info=True,
                )
                plt.close()
        else:
            ...
            logger.debug(
                f"Skipping plot for '{plot_title}': No data found or mismatched epoch counts."
            )

    return generated_plot_paths


def plot_similarity_matrix_snippet(
    similarity_matrix: Optional[np.ndarray],
    report_dir: str,
    epoch_identifier: str,
    max_elements_display: int = 20,
):
    if similarity_matrix is None or similarity_matrix.size == 0:
        logger.warning(
            "Similarity matrix is empty or None. Skipping snippet plot."
        )
        return None
    ...
    num_to_display = min(
        ...
        max_elements_display,
    )
    if num_to_display == 0:
        logger.warning(
            "Cannot display 0x0 similarity matrix snippet. Skipping plot."
        )
        return None
    ...
    sns.heatmap(
        similarity_matrix[:num_to_display, :num_to_display],
    ...
    plt.title(
        f"Similarity Matrix Snippet ({num_to_display}x{num_to_display}) - {epoch_identifier}"
    )
    ...
    safe_epoch_id = (
        re.sub(r"[^\w\s-]", "", epoch_identifier.lower())
        .strip()
        .replace(" ", "_")
    )
    ...
    try:
        ...
        logger.error(
            f"Failed to save similarity matrix plot {filepath}: {e_sim_save}",
            exc_info=True,
        )
        plt.close()
        return None
    ...
    for data_key, plot_title_suffix in length_data_keys.items():
        if data_key in dataset_summary and dataset_summary[data_key]:
            ...
            try:
                plt.savefig(plot_filepath)
                plt.close()
                generated_plot_paths[f"{data_key}_dist_plot"] = (
                    os.path.basename(plot_filepath)
                )
            except Exception as e_len_dist_save:
                logger.error(
                    f"Failed to save length distribution plot {plot_filepath}: {e_len_dist_save}",
                    exc_info=True,
                )
                plt.close()
    return generated_plot_paths
...
def plot_gff_feature_type_distribution(dataset_summary: Dict, report_dir: str):
    ...
    if (
        "gff_feature_type_counts_raw" not in dataset_summary
        or not dataset_summary["gff_feature_type_counts_raw"]
    ):
    ...
    plt.ylabel("Count")
    plt.title(
        f"Distribution of GFF3 Feature Types (Top {min(top_n, len(labels))})"
    )
    plt.xticks(
    ...
    for bar in bars:
        ...
        plt.text(
            ...
            yval + 0.01 * max(values),  # Adjust offset
            f"{int(yval)}",
            ha="center",
            va="bottom",
            fontsize=8,
    ...
    try:
        ...
        logger.error(
            f"Failed to save GFF3 feature type distribution plot {filepath}: {e_gff_plot_save}",
            exc_info=True,
        )
        plt.close()
        return None
...
class PDFReport(FPDF):
    def __init__(
        self, orientation="P", unit="mm", format="A4", title="Training Report"
    ):
    ...
    def setup_fonts(self):
        ...
        try:
            self.add_font(
                self.font_family_main, "", "Helvetica.ttf", uni=True
            )  # Example, ensure ttf path if needed
            self.add_font(
                self.font_family_main, "B", "Helvetica-Bold.ttf", uni=True
            )
            self.add_font(
                self.font_family_main, "I", "Helvetica-Oblique.ttf", uni=True
            )
            self.add_font(self.font_family_mono, "", "Courier.ttf", uni=True)
        except (
            RuntimeError
        ):  # Font file not found, FPDF falls back to core fonts if names match
            logger.debug(
                "FPDF custom font loading skipped, will use core fonts."
            )
    ...
    def header(self):
        ...
        self.cell(
            0,
            10,
            f"{self.report_title_text} - Page {self.page_no()}/{{nb}}",
            0,
            0,
            "C",
        )
    ...
    def chapter_body(self, text: str, font_style="", is_code=False):
        font_family = (
            self.font_family_mono if is_code else self.font_family_main
        )
    ...
    def add_plot(
        self, image_path: str, caption: str = "", width_percent: float = 0.85
    ):
        if not os.path.exists(image_path):
            self.chapter_body(
                f"[Plot Image Missing: {os.path.basename(image_path)}]",
                font_style="I",
            )
            return
        if not PIL:
            self.chapter_body(
                f"[PIL/Pillow not installed, cannot add plot: {os.path.basename(image_path)}]",
                font_style="I",
            )
            return
        ...
        try:
            ...
            logger.warning(
                f"PIL error opening or reading image {image_path}: {e_pil}. Using default aspect ratio."
            )
            img_display_height_mm = img_display_width_mm * (
                3 / 4
            )  # Default aspect ratio

        # Check if image fits, if not, add new page
        if (
            self.get_y() + img_display_height_mm + 20 > self.page_break_trigger
        ):  # 20 for caption and spacing
            self.add_page()
        ...
        x_pos_mm = (
            page_width_mm - img_display_width_mm
        ) / 2 + self.l_margin  # Center image

        try:
            self.image(
                image_path,
                x=x_pos_mm,
                w=img_display_width_mm,
                h=img_display_height_mm,
            )
        except RuntimeError as e_fpdf_img:  # FPDF error (e.g. unsupported image format by FPDF directly)
            logger.error(
                f"FPDF error placing image {image_path}: {e_fpdf_img}"
            )
            self.chapter_body(
                f"[Error displaying plot: {os.path.basename(image_path)} - {e_fpdf_img}]",
                font_style="I",
            )
    ...
    def add_table(
        ...
        title: str = "",
        col_widths_proportions: Optional[List[float]] = None,
        ...
        if (
            ...
            and len(col_widths_proportions) == num_columns
        ):
            effective_col_widths = [
                page_width_usable * prop for prop in col_widths_proportions
            ]
        else:
            effective_col_widths = [
                page_width_usable / num_columns if num_columns > 0 else 0
            ] * num_columns
        ...
        for row_data in data_rows:
            ...
            for i, cell_item in enumerate(row_data):
                ...
                max_chars_in_cell = (
                    int(effective_col_widths[i] / 1.8)
                    if effective_col_widths[i] > 0
                    else 10
                ...
                try:
                    ...
                    logger.error(
                        f"PDF table cell write error: {e_cell}, item (truncated): {item_str[:30]}"
                    )
                    self.cell(
                        effective_col_widths[i],
                        line_height,
    ...
    def add_classification_report_table(self, report_dict: Dict, title: str):
        ...
        class_items = sorted(
            [
                ...
                and label not in ["accuracy", "macro avg", "weighted avg"]
            ],
        ...
        for avg_key in [
            "macro avg",
            "weighted avg",
        ]:  # Ensure consistent order for averages
            if avg_key in report_dict and isinstance(
                report_dict[avg_key], dict
            ):
        ...
        if "accuracy" in report_dict and isinstance(
            report_dict["accuracy"], (float, int)
        ):
            # Calculate total support for accuracy row if not directly available
            total_support_for_accuracy = (
                sum(
                    int(class_metrics.get("support", 0))
                    for _, class_metrics in class_items
                )
                if class_items
                else "N/A"
            )
        ...
        if data_rows:
            self.add_table(
                header,
                data_rows,
                col_widths_proportions=col_widths_props,
                header_fill=True,
            )
        else:
            self.chapter_body(
                "No classification report data available to display."
            )
...
def generate_report(
    ...
    if "plot_paths" not in metrics_history:
        metrics_history["plot_paths"] = defaultdict(list)

    # Generate plots and collect their paths
    trend_plot_paths = plot_metric_trends(metrics_history, report_dir)
    for key, path_val in trend_plot_paths.items():
        metrics_history["plot_paths"][key] = [path_val]  # Store as list

    dist_plot_paths = plot_sequence_length_distributions(
        dataset_summary, report_dir
    )
    for key, path_val in dist_plot_paths.items():
        metrics_history["plot_paths"][key] = [path_val]

    gff_dist_plot_path = plot_gff_feature_type_distribution(
        dataset_summary, report_dir
    )
    if gff_dist_plot_path:
        metrics_history["plot_paths"]["gff_feature_type_dist_plot"] = [
            gff_dist_plot_path
        ]
    ...
    input_file_display = (
        ...
        if args.input_file
        else "N/A (e.g., resumed without new input)"
    )
    ...
    if args.input_type in ["bed", "gff3"] and args.reference_fasta:
        md_report_content += (
            f"Reference FASTA: `{os.path.basename(args.reference_fasta)}`\n"
        )
    md_report_content += f"Main Output Directory: `{args.output_dir}`\n\n"

    md_report_content += (
        f"## 1. Experiment Configuration\n\n| Parameter | Value |\n|---|---|\n"
    )
    ...
    for plot_key, plot_paths_list in metrics_history["plot_paths"].items():
        if (
            "dist_plot" in plot_key or "gff_feature_type_dist_plot" in plot_key
        ) and plot_paths_list:
            plot_title = (
                plot_key.replace("_dist_plot", "")
                .replace("_plot", "")
                .replace("_", " ")
                .title()
                .replace("Dna", "DNA")
    ...
    for count_type_key, count_title in [
        ("biotype_counts", "Biotype"),
        ("species_counts", "Species"),
        ("gff_feature_type_counts_processed", "GFF Feature Type (Processed)"),
    ]:
        if (
            count_type_key in dataset_summary
            and dataset_summary[count_type_key]
        ):
            ...
            if isinstance(counts_data, Counter):
                sorted_counts = counts_data.most_common()
            elif isinstance(counts_data, dict):
                sorted_counts = sorted(
                    counts_data.items(), key=lambda item: item[1], reverse=True
                )
            else:
                sorted_counts = []  # Should not happen
    ...
    num_epochs_completed = metrics_history.get(
        "num_epochs_trained", args.epochs if hasattr(args, "epochs") else 0
    )
    md_report_content += f"## 3. Training & Validation Metric Trends (Across {num_epochs_completed} Epochs)\n\n"
    # Metric trend plots
    for plot_key, plot_paths_list in metrics_history["plot_paths"].items():
        if (
            "trend" in plot_key.lower() and plot_paths_list
        ):  # check "trend" in key name
            plot_title = (
                plot_key.replace("_trend", "")
                .replace("_", " ")
                .title()
                .replace("Dna", "DNA")
            )
    ...
    metrics_to_showcase = [
        ...
        "text_to_dna_R@5",
        "text_to_dna_R@10",
        "knn_biotype_accuracy",
        "knn_biotype_f1_macro",
        "silhouette_score_biotype",
        "davies_bouldin_index_biotype",
        "silhouette_score_species",
        "davies_bouldin_index_species",
    ]
    for metric_name in metrics_to_showcase:
        if metric_name in metrics_history and metrics_history[metric_name]:
            ...
            target_idx = (
                best_val_epoch_idx
                if (0 <= best_val_epoch_idx < len(values_list))
    ...
    knn_reports_list = metrics_history.get(
        "knn_classification_reports_biotype", []
    )
    if knn_reports_list:
        report_idx = (
            best_val_epoch_idx
            if (0 <= best_val_epoch_idx < len(knn_reports_list))
            else (len(knn_reports_list) - 1)
        )
        if report_idx != -1:
            ...
            report_class_items = sorted(
                [
                    (lbl, m_dict)
                    for lbl, m_dict in final_knn_report.items()
                    if isinstance(m_dict, dict)
                    and lbl not in ["accuracy", "macro avg", "weighted avg"]
                ],
                key=lambda x: str(x[0]),
            )
            report_avg_items = [
                (k, final_knn_report[k])
                for k in ["macro avg", "weighted avg"]
                if k in final_knn_report
                and isinstance(final_knn_report[k], dict)
            ]
    ...
    plot_keys_for_final_report = ["knn_biotype_cm"] + [
        f"embedding_plot_{pm}_{lt}"
        for pm in (
            args.plot_methods
            if hasattr(args, "plot_methods")
            else ["pca", "tsne"]
        )
        for lt in ["biotype", "species"]
    ]

    for plot_key in plot_keys_for_final_report:
        if (
            plot_key in metrics_history["plot_paths"]
            and metrics_history["plot_paths"][plot_key]
        ):
            ...
            plot_path_to_display_idx = (
                best_val_epoch_idx
                if (0 <= best_val_epoch_idx < len(plot_paths))
                else -1
            )
            if plot_path_to_display_idx < len(
                plot_paths
            ) and plot_path_to_display_idx >= -len(
                plot_paths
            ):  # check valid index
                plot_path_to_display = plot_paths[plot_path_to_display_idx]
                plot_title_md = (
                    plot_key.replace("_", " ").title().replace("Dna", "DNA")
                )
    ...
    if final_similarity_matrix is not None:
        sim_matrix_plot_path = plot_similarity_matrix_snippet(
            final_similarity_matrix,
            report_dir,
            epoch_identifier=final_eval_epoch_descriptor,
        )
        if sim_matrix_plot_path:
            ...
            metrics_history["plot_paths"][
                "final_similarity_matrix_snippet"
            ] = [sim_matrix_plot_path]

    md_report_filepath = os.path.join(
        report_dir, "training_evaluation_report.md"
    )
    try:
        ...
        logger.error(
            f"Failed to write Markdown report: {e_md_save}", exc_info=True
        )

    # --- PDF Report ---
    if not PIL:
        logger.warning(
            "PIL/Pillow library not found. Skipping PDF report generation."
        )
    else:
        pdf_report_title = (
            f"CLOP Report: {os.path.basename(args.input_file)}"
            if args.input_file
            else "CLOP Report (Resumed)"
        )
        ...
        config_data_pdf = [
            [arg_name, str(arg_val)]
            for arg_name, arg_val in sorted(vars(args).items())
        ]
        pdf.add_table(
            ["Parameter", "Value"],
            config_data_pdf,
            col_widths_proportions=[0.3, 0.7],
        )
        ...
        dataset_info_pdf = [
            [
                key.replace("_", " ").title(),
                str(dataset_summary.get(key, "N/A")),
            ]
            for key in dataset_stat_keys_ordered
        ]
        pdf.add_table(
            ["Statistic", "Value"],
            dataset_info_pdf,
            col_widths_proportions=[0.5, 0.5],
        )

        for plot_key, plot_paths_list in metrics_history["plot_paths"].items():
            if (
                "dist_plot" in plot_key
                or "gff_feature_type_dist_plot" in plot_key
            ) and plot_paths_list:
                plot_title_pdf = (
                    plot_key.replace("_dist_plot", "")
                    .replace("_plot", "")
                ...
                pdf.add_plot(
                    os.path.join(report_dir, plot_paths_list[-1]),
                    caption=plot_title_pdf,
                )
        ...
        for count_type_key, count_title in [
            ...
            ("species_counts", "Species"),
            (
                "gff_feature_type_counts_processed",
                "GFF Feature Type (Processed)",
            ),
        ]:
            if (
                count_type_key in dataset_summary
                ...
                counts_data = dataset_summary[count_type_key]
                if isinstance(counts_data, Counter):
                    sorted_counts_pdf = counts_data.most_common()
                elif isinstance(counts_data, dict):
                    sorted_counts_pdf = sorted(
                        counts_data.items(),
                ...
                counts_data_pdf = [
                    [name, count] for name, count in sorted_counts_pdf[:10]
                ]  # Top 10 for PDF
                if len(sorted_counts_pdf) > 10:
                    counts_data_pdf.append(["... (others)", "..."])
                if counts_data_pdf:  # Only add table if there's data
                    pdf.add_table(
                        [count_title, "Count"],
                        counts_data_pdf,
                        title=f"{count_title} Distribution (Top 10)",
                        col_widths_proportions=[0.7, 0.3],
                        header_fill=True,
                    )
        ...
        pdf.chapter_title(
            f"3. Training & Validation Metric Trends (Across {num_epochs_completed} Epochs)"
        )
        for plot_key, plot_paths_list in metrics_history["plot_paths"].items():
            if "trend" in plot_key.lower() and plot_paths_list:
                plot_title_pdf_trend = (
                    plot_key.replace("_trend", "")
                    .replace("_", " ")
                    .title()
                    .replace("Dna", "DNA")
                ...
                pdf.add_plot(
                    os.path.join(report_dir, plot_paths_list[-1]),
                    caption=plot_title_pdf_trend,
                )

        # Section 4: Final Eval
        pdf.add_page()
        pdf.chapter_title(
            f"4. Detailed Evaluation Metrics & Visualizations ({final_eval_epoch_descriptor})"
        )
        ...
        for metric_name in metrics_to_showcase:
            if metric_name in metrics_history and metrics_history[metric_name]:
                ...
                target_idx_pdf = (
                    best_val_epoch_idx
                    if (0 <= best_val_epoch_idx < len(values_list_pdf))
                    else (len(values_list_pdf) - 1)
                )
                if target_idx_pdf != -1:
                    metric_value_pdf = values_list_pdf[target_idx_pdf]
                    formatted_val_pdf = (
                        f"{metric_value_pdf:.4f}"
                        if isinstance(metric_value_pdf, (float, np.floating))
                        else str(metric_value_pdf)
                    )
                    summary_metrics_pdf_data.append(
                        [
        ...
        if summary_metrics_pdf_data:
            pdf.add_table(
                ...
                col_widths_proportions=[0.6, 0.4],
                header_fill=True,
            )

        if knn_reports_list and report_idx != -1:
            pdf.add_classification_report_table(
                knn_reports_list[report_idx],
                title=f"k-NN Biotype Classification Report ({final_eval_epoch_descriptor})",
            )
        ...
        for plot_key in plot_keys_for_final_report:
            if (
                ...
                plot_path_to_display_idx_pdf = (
                    best_val_epoch_idx
                    if (0 <= best_val_epoch_idx < len(plot_paths_pdf))
                    else -1
                )
                if plot_path_to_display_idx_pdf < len(
                    plot_paths_pdf
                ) and plot_path_to_display_idx_pdf >= -len(plot_paths_pdf):
                    plot_path_to_display_pdf = plot_paths_pdf[
                        plot_path_to_display_idx_pdf
                    ]
                    plot_title_pdf_final = (
                        plot_key.replace("_", " ")
                        .title()
                        .replace("Dna", "DNA")
                    )
                    pdf.add_plot(
                        os.path.join(report_dir, plot_path_to_display_pdf),
                        caption=plot_title_pdf_final,
                    )

        if (
            "final_similarity_matrix_snippet" in metrics_history["plot_paths"]
            and metrics_history["plot_paths"][
                "final_similarity_matrix_snippet"
            ]
        ):
            final_sim_plot_path = metrics_history["plot_paths"][
                "final_similarity_matrix_snippet"
            ][-1]
            pdf.add_plot(
                os.path.join(report_dir, final_sim_plot_path),
                caption=f"Similarity Matrix Snippet ({final_eval_epoch_descriptor})",
            )

        pdf_report_filepath = os.path.join(
            report_dir, "training_evaluation_report.pdf"
        )
        try:
            ...
            logger.error(
                f"Failed to generate PDF report: {e_pdf_save}. PDF may be incomplete or corrupted.",
                exc_info=True,
            )

    logger.info(
        f"Report generation finished in {time.time() - report_gen_start_time:.2f}s."
    )
        ...
        if line_entry.startswith(">"):
            ...
            while re.search(r"\{(\w)\*(\d+)\}", processed_line):
                processed_line = re.sub(
                    r"\{(\w)\*(\d+)\}", expand_matched_sequence, processed_line
                )
...
def export_onnx_to_tfjs(onnx_model_filepath: str, tfjs_output_dir: str):
    if not tensorflow or not onnx or not onnx_tf_prepare or not tensorflowjs:
        logger.error(
            "Missing one or more libraries for TF.js export: tensorflow, onnx, onnx-tf, tensorflowjs. Skipping."
        )
        return
    if not os.path.exists(onnx_model_filepath):
        logger.error(
            f"ONNX model file not found: {onnx_model_filepath}. Skipping TF.js export."
        )
        return

    base_model_name = os.path.splitext(os.path.basename(onnx_model_filepath))[
        0
    ]
    ...
    try:
        logger.info(
            f"Starting TensorFlow.js export for {onnx_model_filepath} to {tfjs_output_dir}"
        )
        ...
        temp_saved_model_dir = tempfile.mkdtemp(
            prefix=f"tf_sm_{base_model_name}_"
        )
        logger.info(
            f"Exporting TensorFlow SavedModel to temporary directory: {temp_saved_model_dir}"
        )
        tf_rep.export_graph(
        ...
        logger.info(
            f"Converting TensorFlow SavedModel to TensorFlow.js format in {tfjs_output_dir}..."
        )
        ...
        tfjs_converter_cmd = [
            sys.executable,
            "-m",
            "tensorflowjs.converters.tf_saved_model_conversion_cli",
        ...
        logger.info(
            f"Executing TensorFlow.js converter command: {' '.join(tfjs_converter_cmd)}"
        )
        conversion_process = subprocess.run(
            tfjs_converter_cmd, capture_output=True, text=True, check=False
        )  # check=False to handle errors manually

        if conversion_process.returncode == 0:
            logger.info(
                f"TensorFlow.js model exported successfully to {tfjs_output_dir}"
            )
        else:
            logger.error(
                f"TensorFlow.js conversion failed with return code {conversion_process.returncode}."
            )

        if (
            conversion_process.stdout
        ):  # Log STDOUT regardless of success for debugging
            logger.debug(
                f"TFJS Converter STDOUT:\n{conversion_process.stdout}"
            )
        if (
            conversion_process.stderr and conversion_process.returncode != 0
        ):  # Log STDERR only on error
            logger.error(
                f"TFJS Converter STDERR:\n{conversion_process.stderr}"
            )

    except Exception as e_tfjs:
        logger.error(
            f"An error occurred during TensorFlow.js export process: {e_tfjs}",
            exc_info=True,
        )
    finally:
        if temp_saved_model_dir and os.path.exists(temp_saved_model_dir):
            try:
                ...
                logger.info(
                    f"Cleaned up temporary SavedModel directory: {temp_saved_model_dir}"
                )
            except Exception as e_cleanup:
                logger.error(
                    f"Failed to clean up temporary directory {temp_saved_model_dir}: {e_cleanup}"
                )
        logger.info(
            f"TensorFlow.js export process finished in {time.time() - export_tfjs_start_time:.2f}s."
        )
...
def export_encoders_to_onnx(
    ...
    text_tokenizer: TextTokenizer,
    output_dir: str,
    args_from_run: argparse.Namespace,
):
    """
    ...
    export_dummy_dna_len = (
        args_from_run.max_dna_len if args_from_run.max_dna_len > 0 else 128
    )
    export_dummy_text_len = (
        args_from_run.max_text_len if args_from_run.max_text_len > 0 else 64
    )

    # The 'lengths' tensor must be on the CPU for the original torch.onnx.export call
    dummy_dna_tokens_export = torch.randint(
        0,
        dna_tokenizer.get_vocab_size(),
        (1, export_dummy_dna_len),
        dtype=torch.long,
    ).to(device)
    dummy_dna_lengths_export = torch.tensor(
        [export_dummy_dna_len // 2], dtype=torch.long
    ).cpu()
    dummy_text_tokens_export = torch.randint(
        0,
        text_tokenizer.get_vocab_size(),
        (1, export_dummy_text_len),
        dtype=torch.long,
    ).to(device)
    dummy_text_lengths_export = torch.tensor(
        [export_dummy_text_len // 2], dtype=torch.long
    ).cpu()
    ...
    if isinstance(model.dna_encoder, LSTMEncoder):
        logger.info(
            "Wrapping DNA LSTM encoder in ONNX-friendly container for export."
        )
    ...
    try:
        torch.onnx.export(
            dna_encoder_for_export,
            (
                dummy_dna_tokens_export,
                dummy_dna_lengths_export.to(device),
            ),  # Pass lengths on same device
            dna_encoder_onnx_path,
            input_names=["dna_tokens", "dna_lengths"],
            output_names=["dna_embedding"],
            dynamic_axes={
                "dna_tokens": {0: "batch_size", 1: "sequence_length"},
        ...
        logger.error(
            f"Failed to export DNA encoder to ONNX: {e_onnx_dna}",
            exc_info=True,
        )
    ...
    if isinstance(model.text_encoder, LSTMEncoder):
        logger.info(
            "Wrapping Text LSTM encoder in ONNX-friendly container for export."
        )
    ...
    try:
        torch.onnx.export(
            text_encoder_for_export,
            (
                dummy_text_tokens_export,
                dummy_text_lengths_export.to(device),
            ),  # Pass lengths on same device
        ...
        logger.error(
            f"Failed to export text encoder to ONNX: {e_onnx_text}",
            exc_info=True,
        )
    ...
    web_config = {
        ...
        "k": getattr(
            dna_tokenizer, "k", None
        ),  # Safely get k-mer size if applicable
    }
    ...
    logger.info(
        f"ONNX export finished in {time.time() - onnx_export_start_time:.2f}s."
    )
...
def convert_checkpoint_to_onnx(
    ...
    if not chkp_args_dict:
        logger.critical(
            "Checkpoint does not contain 'args'. Cannot determine model architecture."
        )
        return
    ...
    logger.info(
        f"Checkpoint was trained with encoder type: '{encoder_type_from_chkp}'"
    )

    # Load tokenizers
    checkpoint_dir = os.path.dirname(checkpoint_path)
    dna_tok_path = load_dna_tok_path or os.path.join(
        checkpoint_dir, "dna_tokenizer_vocab.json"
    )
    text_tok_path = load_text_tok_path or os.path.join(
        checkpoint_dir, "text_tokenizer_vocab.json"
    )

    if not os.path.exists(dna_tok_path) or not os.path.exists(text_tok_path):
        logger.critical(
            f"Tokenizer vocab files not found. Expected at '{dna_tok_path}' and '{text_tok_path}'."
        )
        return

    dna_tokenizer_cls = (
        KmerDNATokenizer
        if chkp_args.dna_tokenizer_type == "kmer"
        else CharDNATokenizer
    )
    ...
    if any(key.startswith("_orig_mod.") for key in state_dict.keys()):
        logger.info(
            "Checkpoint appears to be from a torch.compile'd model. Cleaning state_dict keys."
        )
        state_dict = {
...
def convert_csv_to_parquet(csv_filepath: str):
    ...
    if pl is None:
        logger.critical(
            "The 'polars' library is required for this conversion. Please install it (`pip install polars`) and try again."
        )
        return

    if not csv_filepath.lower().endswith(".csv"):
        logger.warning(
            f"Input file '{csv_filepath}' does not have a .csv extension. Proceeding anyway."
        )
    ...
    try:
        ...
        logger.critical(
            f"An error occurred during conversion: {e}", exc_info=True
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train DNA-Annotation CLIP-like model.",
    ...
    io_group.add_argument(
        ...
        type=str,
        required=False,
        help="Path to FASTA/BED/GFF3 file (gzipped ok).",
    )
    io_group.add_argument(
        "--input_type",
        type=str,
        choices=["fasta", "bed", "gff3"],
        default="fasta",
    ...
    io_group.add_argument(
        "--reference_fasta",
        type=str,
        help="Reference FASTA (required for BED and GFF3 input types).",
    )
    io_group.add_argument(
        "--output_dir",
        type=str,
        default="dna_clip_output",
        help="Directory for all outputs.",
    )
    io_group.add_argument(
        "--load_model_path",
        type=str,
        help="Path to load pre-trained model state_dict (weights only).",
    )
    io_group.add_argument(
        "--dna_tokenizer_type",
        type=str,
        default="kmer",
        choices=["kmer", "char"],
        help="Type of DNA tokenizer to use.",
    )
    io_group.add_argument(
        "--load_dna_tokenizer_path",
        type=str,
        help="Path to load pre-existing DNA tokenizer vocabulary.",
    )
    io_group.add_argument(
        "--load_text_tokenizer_path",
        type=str,
        help="Path to load pre-existing text tokenizer vocabulary.",
    )
    io_group.add_argument(
        "--resume_from_checkpoint",
        type=str,
        help="Path to a checkpoint file to resume full training state.",
    )
    io_group.add_argument(
        "--gff_feature_types",
        type=str,
        default=",".join(DEFAULT_GFF_FEATURE_TYPES),
        help="Comma-separated list of GFF3 feature types to process (e.g., gene,mRNA,ncRNA_gene). Used if --input_type is gff3.",
    )
    ...
    model_group.add_argument(
        "--kmer_k",
        type=int,
        default=DEFAULT_KMER_K,
        help="K-mer size (if using k-mer DNA tokenizer).",
    )
    model_group.add_argument(
        "--embedding_dim",
        type=int,
    ...
    model_group.add_argument(
        "--dropout", type=float, default=0.1, help="Dropout rate in model."
    )
    model_group.add_argument(
        "--initial_temperature",
        type=float,
        default=DEFAULT_TEMPERATURE,
        help="Initial temperature for contrastive loss scaling.",
    )
    ...
    train_group.add_argument(
        ...
        type=int,
        default=4096,
        help="Maximum length for DNA sequences (tokens). 0 for dynamic (padded per batch).",
    )
    train_group.add_argument(
        "--max_text_len",
        type=int,
        default=256,
        help="Maximum length for text annotations (tokens). 0 for dynamic.",
    )
    train_group.add_argument(
        "--validation_split",
        type=float,
    ...
    train_group.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum frequency for tokens in vocabulary building.",
    )
    train_group.add_argument(
        "--optimizer_type",
        type=str,
    ...
    train_group.add_argument(
        ...
        type=float,
        default=0.01,
        help="Weight decay (L2 penalty), primarily for AdamW.",
    )
    train_group.add_argument(
        "--lr_scheduler_patience",
        type=int,
        default=3,
        help="Patience for ReduceLROnPlateau scheduler (epochs). 0 to disable.",
    )
    train_group.add_argument(
        "--lr_scheduler_factor",
        type=float,
        default=0.1,
        help="Factor by which LR is reduced by scheduler.",
    )
    train_group.add_argument(
        "--early_stopping_patience",
        type=int,
        default=5,
        help="Patience for early stopping (epochs). 0 to disable.",
    )
    train_group.add_argument(
        "--use_amp",
        action="store_true",
        help="Use Automatic Mixed Precision (AMP) for CUDA training.",
    )
    train_group.add_argument(
        "--use_torch_compile",
        action="store_true",
    ...
    export_group.add_argument(
        "--export_onnx",
        action="store_true",
        help="Export encoders to ONNX format after training.",
    )
    export_group.add_argument(
        "--export_tfjs",
        action="store_true",
        help="Export ONNX encoders to TensorFlow.js format after training (requires ONNX export).",
    )
    export_group.add_argument(
        "--export_embeddings_format",
        type=str,
        choices=["csv", "parquet", "both", "none"],
        default="both",
        help="Format for exporting final embeddings.",
    )
    export_group.add_argument(
        "--convert_checkpoint_to_onnx",
        type=str,
        help="Path to a model checkpoint (.pth) to convert to ONNX format. Skips training.",
    )
    export_group.add_argument(
        "--convert_csv_to_parquet",
        type=str,
        help="Path to an embeddings CSV file to convert to Parquet format. Skips training.",
    )
    export_group.add_argument(
        "--encoder_type",
        type=str,
        ...
        default="lstm",
        help="Model encoder architecture to use for conversion or training.",
    )
    ...
    misc_group.add_argument(
        "--run_dummy_example",
        action="store_true",
        help="Run with a small, internally generated dummy dataset for testing.",
    )
    misc_group.add_argument(
        "--test_suite",
        action="store_true",
    ...
    misc_group.add_argument(
        ...
        help="Methods for embedding visualization.",
    )
    misc_group.add_argument(
        "--log_level",
        type=str,
    ...
    misc_group.add_argument(
        ...
        type=int,
        default=None,
        help="Specific GPU ID to use (e.g., 0, 1). None for automatic selection.",
    )
    misc_group.add_argument(
        "--force_cpu",
        action="store_true",
    ...
    misc_group.add_argument(
        "--diagnose_cuda",
        action="store_true",
        help="Run CUDA diagnostics and exit.",
    )
    ...
    if args.convert_csv_to_parquet:
        if not os.path.exists(args.convert_csv_to_parquet):
            logger.critical(
                f"Input CSV file not found: {args.convert_csv_to_parquet}"
            )
            sys.exit(1)
    ...
    if args.convert_checkpoint_to_onnx:
        if not os.path.exists(args.convert_checkpoint_to_onnx):
            logger.critical(
                f"Checkpoint file not found: {args.convert_checkpoint_to_onnx}"
            )
            sys.exit(1)
        convert_checkpoint_to_onnx(
    ...
    if args.run_dummy_example:
        ...
        base_output_dir_dummy = (
            args.output_dir
        )  # User specified or default "dna_clip_output"
        args.input_file = os.path.join(
            base_output_dir_dummy, "dummy_data.fasta.gz"
        )  # Store dummy data inside this base
        args.output_dir = os.path.join(
            base_output_dir_dummy, "dummy_model_outputs"
        ...
        args.hidden_dim = 32
        args.validation_split = 0.40
        args.early_stopping_patience = 2
        args.lr_scheduler_patience = 1
        args.num_workers = 0

        os.makedirs(
            base_output_dir_dummy, exist_ok=True
        )  # Ensure base dir for dummy input exists
        ...
        dummy_fasta_content_expanded = expand_fasta_curly_braces(
            dummy_fasta_str_template
        )
        ...
        if args.dna_tokenizer_type == "kmer" and args.kmer_k > 3:
            logger.info(
                f"For dummy run with k-mer tokenizer, reducing kmer_k to 3 for short sequences."
            )
            args.kmer_k = 3
        ...
        parser.error(
            "Either --input_file or --resume_from_checkpoint must be provided for a non-dummy run."
        )
        ...
        if args.gpu_id is not None:
            ...
            if torch.cuda.is_available():
                try:
                    ...
                    del test_tensor
                    logger.info(
                        f"Successfully initialized CUDA device {args.gpu_id}: {torch.cuda.get_device_name(args.gpu_id)}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to use GPU {args.gpu_id}: {e}. Falling back to CPU."
                    )
                    device = torch.device("cpu")
            else:
            ...
            if torch.cuda.is_available():
                try:
                    ...
                    del test_tensor
                    logger.info(
                        f"Successfully initialized CUDA device: {torch.cuda.get_device_name()}"
                    )
                except Exception as e:
                    logger.warning(
                        f"CUDA is available but initialization failed: {e}. Using CPU."
                    )
                    device = torch.device("cpu")
            else:
                device = torch.device("cpu")
    ...
    if args.use_amp and device.type != "cuda":
        logger.warning(
            "AMP (Automatic Mixed Precision) requires CUDA. Disabling AMP."
        )
        args.use_amp = False
    ...
    if device.type == "cuda" and args.use_amp:
        ...
        logger.info(
            "CUDA available. AMP enabled. CuDNN benchmark mode set to True."
        )
    ...
    try:
        ...
        logger.critical(f"Failed to create output directories: {e_mkdir}")
        sys.exit(1)
    ...
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(args.seed)  # For multi-GPU if used

    # Device and AMP setup
    if args.use_amp and not torch.cuda.is_available():
        logger.warning(
            "AMP (Automatic Mixed Precision) was requested, but CUDA is not available. Disabling AMP."
        )
        args.use_amp = False

    if torch.cuda.is_available() and args.use_amp:
        torch.backends.cudnn.benchmark = (
            True  # Can speed up training if input sizes don't vary much
        )
        logger.info(
            "CUDA available. AMP enabled. CuDNN benchmark mode set to True."
        )
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(
        f"Using device: {device}. AMP active: {args.use_amp and device.type == 'cuda'}"
    )
    ...
    metrics_history["plot_paths"] = defaultdict(
        list
    )  # To store paths of generated plots
    metrics_history[
        "knn_classification_reports_biotype"
    ] = []  # Store full dicts for reports
    ...
    if args.input_file:
        ...
        if args.input_type == "fasta":
            ...
            if not args.reference_fasta:
                logger.critical(
                    "--reference_fasta is required for --input_type gff3."
                )
                sys.exit(1)
            target_gff_types = [
                s.strip()
                for s in args.gff_feature_types.split(",")
                if s.strip()
            ]
            logger.info(
                f"Processing GFF3 features of types: {target_gff_types}"
            )
            parsed_data_raw = parse_gff3(
                args.input_file, args.reference_fasta, target_gff_types
            )

        # Populate sequences_data, annotations_data, metadata_list from parsed_data_raw
        for item_tuple in parsed_data_raw:
            id_val, seq_val, meta_val = (
                item_tuple[0],
                item_tuple[1],
                item_tuple[2],
            )
            ann_text_val = (
                item_tuple[3]
                if len(item_tuple) > 3 and item_tuple[3] is not None
                else ""
            )
        ...
        if not sequences_data:
            logger.critical(
                f"No sequences loaded from {args.input_file}. Check file, format, and GFF feature types if applicable."
            )
            sys.exit(1)

        logger.info(
            f"Successfully loaded {len(sequences_data)} sequence-annotation pairs in {time.time() - data_loading_start_time:.2f}s."
        )

        # Log a few examples
        logger.info("Example loaded data (first 3 entries):")
        for i_ex in range(min(3, len(sequences_data))):
            seq_snippet = sequences_data[i_ex][:50] + (
                "..." if len(sequences_data[i_ex]) > 50 else ""
            )
            logger.info(
                f"  Example {i_ex + 1}: Seq='{seq_snippet}', Ann='{annotations_data[i_ex][:100]}...'"
            )
        ...
        if args.dna_tokenizer_type == "kmer":
            ...
            raise ValueError(
                f"Unsupported DNA tokenizer type: {args.dna_tokenizer_type}"
            )

        if args.load_dna_tokenizer_path and os.path.exists(
            args.load_dna_tokenizer_path
        ):
            logger.info(
                f"Loading DNA tokenizer from: {args.load_dna_tokenizer_path}"
            )
            dna_tokenizer = dna_tokenizer_cls.load_vocab(
                args.load_dna_tokenizer_path
            )
        else:
            logger.info(
                f"Building DNA tokenizer ({args.dna_tokenizer_type}) from data..."
            )
            dna_tokenizer = build_vocab_from_data(
                sequences_data,
                dna_tokenizer_cls,
                args.min_token_freq,
                dna_tokenizer_args,
        ...
        if (
            isinstance(dna_tokenizer, KmerDNATokenizer)
            and hasattr(dna_tokenizer, "k")
            and dna_tokenizer.k != args.kmer_k
        ):
            logger.info(
                f"K-mer size updated to {dna_tokenizer.k} from loaded DNA tokenizer."
            )
            args.kmer_k = dna_tokenizer.k

        # Text Tokenizer
        if args.load_text_tokenizer_path and os.path.exists(
            args.load_text_tokenizer_path
        ):
            logger.info(
                f"Loading text tokenizer from: {args.load_text_tokenizer_path}"
            )
            text_tokenizer = TextTokenizer.load_vocab(
                args.load_text_tokenizer_path
            )
        else:
            logger.info("Building text tokenizer from data...")
            text_tokenizer = build_vocab_from_data(
                annotations_data, TextTokenizer, args.min_token_freq
            )

        # Save tokenizers
        dna_tokenizer.save_vocab(
            os.path.join(args.output_dir, "dna_tokenizer_vocab.json")
        )
        text_tokenizer.save_vocab(
            os.path.join(args.output_dir, "text_tokenizer_vocab.json")
        )

        logger.info(
            f"DNA Tokenizer ({args.dna_tokenizer_type}): Vocab Size = {dna_tokenizer.get_vocab_size()}, PAD ID = {dna_tokenizer.pad_token_id}"
        )
        if isinstance(dna_tokenizer, KmerDNATokenizer):
            logger.info(f"Effective K-mer k = {dna_tokenizer.k}")
        logger.info(
            f"Text Tokenizer: Vocab Size = {text_tokenizer.get_vocab_size()}, PAD ID = {text_tokenizer.pad_token_id}"
        )
        ...
        if args.validation_split > 0 and args.validation_split < 1:
            try:
                ...
                biotype_labels_for_split = [
                    meta.get("biotype", "unknown") for meta in metadata_list
                ]
                ...
                valid_for_stratify = (
                    all(
                        c >= min_samples_per_class_for_stratify
                        for c in label_counts_split.values()
                    )
                ...
                if valid_for_stratify:
                    train_indices, val_indices = train_test_split(
                        ...
                        stratify=biotype_labels_for_split,
                    )
                else:
                    logger.warning(
                        "Not enough samples or classes for stratified split based on biotype. Using random split."
                    )
                    train_indices, val_indices = train_test_split(
                        all_indices,
                        test_size=args.validation_split,
                ...
                ValueError
            ) as e_split:  # Catch errors from train_test_split itself
                logger.warning(
                    f"Stratified split failed ({e_split}). Using random split."
                )
                train_indices, val_indices = train_test_split(
                    all_indices,
                    test_size=args.validation_split,
            ...
            logger.info(
                "Validation split is 0 or invalid, using all data for training (no validation set)."
            )

        logger.info(
            f"Data split: {len(train_indices)} training samples, {len(val_indices)} validation samples."
        )
        ...
        train_dl = create_dataloader(
            ...
            args.num_workers,
            shuffle=True,
            dna_pad_id=dna_tokenizer.pad_token_id,
        ...
        if val_indices:
            ...
            val_dl = create_dataloader(
                ...
                pin_memory=(device.type == "cuda"),
                drop_last=False,
            )  # Do not drop_last for validation
        ...
        dataset_summary = {
            ...
            "biotype_counts": Counter(
                m.get("biotype", "unknown") for m in metadata_list
            ),
            "species_counts": Counter(
                m.get("species", "unknown") for m in metadata_list
            ),
            "dna_lengths_raw": [
                len(s) for s in sequences_data
            ],  # Lengths before tokenization
            "text_lengths_raw_words": [
                len(a.split()) for a in annotations_data
            ],  # Approx word counts
        }
        if args.input_type == "gff3":
            ...
            global dataset_summary_temp_storage
            if (
                "dataset_summary_temp_storage" in globals()
                and "gff_feature_type_counts_raw"
                in dataset_summary_temp_storage
            ):
                dataset_summary["gff_feature_type_counts_raw"] = (
                    dataset_summary_temp_storage["gff_feature_type_counts_raw"]
                )
            # Count GFF feature types that were actually processed (after filtering by target_feature_types)
            dataset_summary["gff_feature_type_counts_processed"] = Counter(
                m.get("gff_feature_type", "unknown_gff_type")
                for m in metadata_list
            )

    elif args.resume_from_checkpoint:
        logger.info(
            "Resuming training. Tokenizers will be loaded based on checkpoint's context or --load paths."
        )
        checkpoint_dir = os.path.dirname(
            args.resume_from_checkpoint
        )  # Assumes tokenizers are in same dir

        dna_tokenizer_cls_resume = (
            KmerDNATokenizer
            if args.dna_tokenizer_type == "kmer"
            else CharDNATokenizer
        )
        dna_tok_path_resume = args.load_dna_tokenizer_path or os.path.join(
            checkpoint_dir, "dna_tokenizer_vocab.json"
        )
        if os.path.exists(dna_tok_path_resume):
            dna_tokenizer = dna_tokenizer_cls_resume.load_vocab(
                dna_tok_path_resume
            )
            if isinstance(dna_tokenizer, KmerDNATokenizer) and hasattr(
                dna_tokenizer, "k"
            ):
                args.kmer_k = dna_tokenizer.k
        else:
            logger.critical(
                f"DNA tokenizer not found at {dna_tok_path_resume} for resumed run. Exiting."
            )
            sys.exit(1)

        text_tok_path_resume = args.load_text_tokenizer_path or os.path.join(
            checkpoint_dir, "text_tokenizer_vocab.json"
        )
        if os.path.exists(text_tok_path_resume):
            ...
            logger.critical(
                f"Text tokenizer not found at {text_tok_path_resume} for resumed run. Exiting."
            )
            sys.exit(1)

        if not args.input_file:
            logger.warning(
                "Resuming without --input_file. DataLoaders (train_dl, val_dl) will be None. This is only suitable for loading a model for inference/export, not continued training."
            )
    else:  # Should be caught by arg parsing logic earlier
        logger.critical(
            "No input data source (file or checkpoint) and not a dummy run. Exiting."
        )
        sys.exit(1)

    if (
        not dna_tokenizer or not text_tokenizer
    ):  # Should have been initialized by now
        logger.critical(
            "Tokenizers not initialized. This indicates a logic error. Exiting."
        )
        sys.exit(1)
    ...
    logger.info(
        f"Model initialized in {time.time() - model_init_start_time:.2f}s."
    )

    if args.use_torch_compile:
        if hasattr(torch, "compile") and sys.version_info >= (
            3,
            8,
        ):  # torch.compile needs Py3.8+ generally
            ...
            try:
                ...
                logger.info(
                    f"torch.compile() applied successfully in {time.time() - compile_start_time:.2f}s."
                )
            except Exception as e_compile:
                logger.warning(
                    f"torch.compile() failed: {e_compile}. Proceeding without it.",
                    exc_info=True,
                )
        else:
            logger.warning(
                "torch.compile requested but not available or fully supported in this PyTorch/Python version."
            )
    ...
    scheduler = None
    if (
        args.lr_scheduler_patience > 0 and val_dl
    ):  # Scheduler needs validation data
        ...
        logger.info(
            f"ReduceLROnPlateau learning rate scheduler active with patience {args.lr_scheduler_patience}."
        )
    elif args.lr_scheduler_patience > 0 and not val_dl:
        logger.warning(
            "LR scheduler requested, but no validation data is available. Scheduler will be inactive."
        )
    ...
    if args.resume_from_checkpoint and os.path.exists(
        ...
        try:
            ...
            if any(key.startswith("_orig_mod.") for key in state_dict.keys()):
                logger.info(
                    "Resuming from a torch.compile'd model. Cleaning state_dict keys."
                )
                state_dict = {
                    key.replace("_orig_mod.", ""): value
                    for key, value in state_dict.items()
            ...
            for key, val_list in chkpt_metrics_history.items():
                if key == "plot_paths":  # defaultdict of lists
                    for sub_key, paths in val_list.items():
                        metrics_history[key][sub_key].extend(paths)
                elif (
                    key == "knn_classification_reports_biotype"
                ):  # list of dicts
            ...
            current_lr_resumed = (
                ...
                if "optimizer_state_dict" in checkpoint
                else "N/A (Optimizer not loaded from chkpt)"
            )
            logger.info(
                f"Resumed from epoch {start_epoch - 1}. Best validation loss: {best_val_loss:.4f}. Current LR: {current_lr_resumed}"
            )

        except Exception as e_chkpt_load:
            logger.error(
                f"Failed to load checkpoint from {args.resume_from_checkpoint}: {e_chkpt_load}. Starting fresh training.",
                exc_info=True,
            )
            start_epoch = 0
            best_val_loss = float("inf")
            metrics_history = defaultdict(list)
            metrics_history["plot_paths"] = defaultdict(list)
            metrics_history["knn_classification_reports_biotype"] = []  # Reset

    elif (
        args.load_model_path
    ):  # Load only model weights, not full training state
        ...
        try:
            ...
            logger.warning(
                f"Strict loading of model weights failed: {e_strict_load}. Attempting non-strict loading (strict=False)..."
            )
            try:
                model.load_state_dict(
                    torch.load(
                ...
                logger.info(
                    "Model weights loaded successfully (strict=False). Some layers might be unmatched or differently sized."
                )
            except Exception as e_non_strict_load:
                logger.error(
                    f"Non-strict loading of model weights also failed: {e_non_strict_load}. Model will use fresh random weights.",
                    exc_info=True,
                )
    ...
    if not train_dl:
        logger.warning(
            "Training DataLoader (train_dl) is not available. Skipping training loop."
        )
    else:
        logger.info(
            f"Starting training from epoch {start_epoch + 1} up to {args.epochs} epochs..."
        )
        training_loop_start_time = time.time()
        best_epoch_idx = metrics_history.get(
            "best_epoch_idx", -1
        )  # From resumed history if any

        for epoch in range(start_epoch, args.epochs):
            metrics_history["num_epochs_trained"] = (
                epoch + 1
            )  # Record actual number of epochs run so far
            ...
            logger.info(
                f"Epoch {epoch + 1}/{args.epochs} Summary - Train Loss: {avg_train_loss:.4f}, Avg Temp: {avg_temp:.4f}, Avg Grad Norm: {avg_grad_norm:.4f}, LR: {optimizer.param_groups[0]['lr']:.2e}"
            )
            ...
            if val_dl:
                ...
                current_val_loss = val_epoch_metrics.get(
                    "avg_loss", float("inf")
                )

                if not (
                    np.isnan(current_val_loss) or np.isinf(current_val_loss)
                ):
                    if scheduler:
                        scheduler.step(current_val_loss)  # LR scheduler step
                    ...
                    if current_val_loss < best_val_loss:
                        ...
                        best_model_checkpoint_path = os.path.join(
                            args.output_dir, "best_model_checkpoint.pth"
                        )
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to save best model checkpoint: {e_save_best_chkpt}",
                                exc_info=True,
                            )
                        else:
                            logger.info(
                                f"New best model checkpoint saved (Validation Loss: {best_val_loss:.4f} at Epoch {epoch + 1}) to {best_model_checkpoint_path}"
                            )
                        epochs_without_improvement = 0

                        # Plot embeddings for the best model epoch
                        if (
                            dna_val_embeds is not None
                            and dna_val_embeds.shape[0] > 1
                        ):
                            for plot_method_name in args.plot_methods:
                                if plot_method_name == "umap" and not umap:
                                    continue  # Skip if UMAP not installed

                                plot_filename_bio = f"dna_embeddings_{plot_method_name}_biotype_epoch_{epoch + 1}.png"
                                plot_filepath_bio = os.path.join(
                                    report_dir, plot_filename_bio
                                )
                                if plot_embeddings(
                                    dna_val_embeds,
                                    biotypes_val,
                                    f"DNA Embeddings ({plot_method_name.upper()}) by Biotype (Epoch {epoch + 1})",
                                    plot_filepath_bio,
                                    method=plot_method_name,
                                ):
                                    metrics_history["plot_paths"][
                                        f"embedding_plot_{plot_method_name}_biotype"
                                    ].append(plot_filename_bio)

                                plot_filename_sp = f"dna_embeddings_{plot_method_name}_species_epoch_{epoch + 1}.png"
                                plot_filepath_sp = os.path.join(
                                    report_dir, plot_filename_sp
                                )
                                if plot_embeddings(
                                    dna_val_embeds,
                                    species_val,
                                    f"DNA Embeddings ({plot_method_name.upper()}) by Species (Epoch {epoch + 1})",
                                    plot_filepath_sp,
                                    method=plot_method_name,
                                ):
                                    metrics_history["plot_paths"][
                                        f"embedding_plot_{plot_method_name}_species"
                                    ].append(plot_filename_sp)
                    else:  # Validation loss did not improve
                        epochs_without_improvement += 1
                        logger.info(
                            f"Validation loss did not improve for {epochs_without_improvement} epoch(s). Best validation loss remains: {best_val_loss:.4f} (from Epoch {best_epoch_idx + 1})"
                        )
                else:  # NaN/Inf validation loss
                    epochs_without_improvement += 1
                    logger.warning(
                        f"Validation loss was NaN/Inf for epoch {epoch + 1}. Counting as no improvement."
                    )
                ...
                if (
                    ...
                    logger.info(
                        f"Early stopping triggered after {epochs_without_improvement} epochs without validation loss improvement."
                    )
            ...
            if (epoch + 1) % 5 == 0 or (epoch + 1) == args.epochs:
                ...
                try:
                    torch.save(
                        periodic_checkpoint_data, periodic_checkpoint_path
                    )
                except Exception as e_save_periodic_chkpt:
                    logger.error(
                        f"Failed to save periodic checkpoint: {e_save_periodic_chkpt}",
                        exc_info=True,
                    )
                else:
                    logger.info(
                        f"Periodic checkpoint saved: {periodic_checkpoint_path}"
                    )

        logger.info(
            f"Training loop finished in {time.time() - training_loop_start_time:.2f}s."
        )
    ...
    final_model_to_use_path = os.path.join(
        args.output_dir, "best_model_checkpoint.pth"
    )
    descriptor_for_final_eval = (
        f"Best Model (Epoch {best_epoch_idx + 1} - Val Loss: {best_val_loss:.4f})"
    ...
    if not (os.path.exists(final_model_to_use_path) and best_epoch_idx != -1):
        ...
        last_completed_epoch = metrics_history.get(
            "num_epochs_trained", start_epoch
        )
        last_epoch_chkpt_num = (
            last_completed_epoch if last_completed_epoch > 0 else start_epoch
        )  # Use start_epoch if no training happened

        final_model_to_use_path = os.path.join(
            args.output_dir, f"checkpoint_epoch_{last_epoch_chkpt_num}.pth"
        )
        ...
        if not os.path.exists(final_model_to_use_path):
            ...
            descriptor_for_final_eval = (
                "Current In-Memory Model (No Checkpoint Loaded for Final Eval)"
            )

    if final_model_to_use_path and os.path.exists(final_model_to_use_path):
        logger.info(
            f"Loading model from {final_model_to_use_path} for final evaluation and export ({descriptor_for_final_eval})."
        )
        try:
            final_checkpoint = torch.load(
                final_model_to_use_path,
                map_location=device,
                weights_only=False,
            )
            ...
            logger.error(
                f"Failed to load model from {final_model_to_use_path} for final ops: {e_final_load}. Using current in-memory model state.",
                exc_info=True,
            )
            descriptor_for_final_eval = (
                "Current In-Memory Model (Fallback due to load error)"
            )
    else:
        logger.info(
            f"Using {descriptor_for_final_eval} (current in-memory state) for final evaluation and export."
        )
    ...
    final_eval_dataset_name = (
        "Validation Set" if val_dl else "Training Set (used for final eval)"
    )
    ...
    if not final_evaluation_dl:
        logger.warning(
            "No DataLoader available for final evaluation or embedding export. Skipping these steps."
        )
    else:
        logger.info(
            f"--- Performing Final Evaluation on {final_eval_dataset_name} using {descriptor_for_final_eval} ---"
        )
        ...
        final_eval_metrics_agg["plot_paths"] = defaultdict(
            list
        )  # Needed by evaluate
        final_eval_metrics_agg[
            "knn_classification_reports_biotype"
        ] = []  # Needed by evaluate

        # Determine epoch number for plot filenames (best epoch or last trained epoch)
        epoch_for_final_eval_plots = (
            best_epoch_idx
            if "Best Model" in descriptor_for_final_eval
            and best_epoch_idx != -1
            else metrics_history.get("num_epochs_trained", 0) - 1
        )
        epoch_for_final_eval_plots = max(
            0, epoch_for_final_eval_plots
        )  # Ensure non-negative
        ...
        if (
            final_dna_embeddings is not None
            and final_text_embeddings is not None
        ):
            final_similarity_matrix_output = (
                final_dna_embeddings @ final_text_embeddings.T
            )  # For report snippet

        # Export Embeddings
        if (
            args.export_embeddings_format != "none"
            and final_dna_embeddings is not None
            and final_text_embeddings is not None
        ):
            logger.info(
                f"Exporting {final_dna_embeddings.shape[0]} DNA and Text embeddings from {final_eval_dataset_name}..."
            )
            ...
            final_ds_metadata = final_evaluation_dl.dataset.metadata[
                :num_embeddings
            ]
            ...
            embeddings_df_data = {
                "id": [
                    m.get("id", f"item_{i}")
                    for i, m in enumerate(final_ds_metadata)
                ],
            ...
            for i_emb_dim in range(final_dna_embeddings.shape[1]):
                embeddings_df_data[f"dna_emb_{i_emb_dim}"] = (
                    final_dna_embeddings[:, i_emb_dim]
                )
            for i_emb_dim in range(final_text_embeddings.shape[1]):
                embeddings_df_data[f"text_emb_{i_emb_dim}"] = (
                    final_text_embeddings[:, i_emb_dim]
                )
            ...
            if args.export_embeddings_format in ["csv", "both"]:
                csv_filepath = os.path.join(
                    args.output_dir, "final_embeddings.csv"
                )
                embeddings_df.to_csv(csv_filepath, index=False)
                logger.info(
                    f"Embeddings exported in CSV format to: {csv_filepath}"
                )
            if args.export_embeddings_format in ["parquet", "both"]:
                if pl:  # Polars is available
                    parquet_filepath = os.path.join(
                        args.output_dir, "final_embeddings.parquet"
                    )
                    try:
                        pl_df = pl.from_pandas(embeddings_df)
                        pl_df.write_parquet(parquet_filepath)
                        logger.info(
                            f"Embeddings exported in Parquet format to: {parquet_filepath}"
                        )
                    except Exception as e_parquet:
                        logger.error(
                            f"Failed to export embeddings to Parquet: {e_parquet}",
                            exc_info=True,
                        )
                else:  # Polars not available
                    logger.warning(
                        "Polars library not installed. Skipping Parquet export for embeddings."
                    )
        else:
            logger.info(
                "Skipping embedding export (format='none' or embeddings not available)."
            )
    ...
    export_dummy_dna_len = (
        ds_max_dna_len
        if "ds_max_dna_len" in locals() and ds_max_dna_len is not None
        else args.max_dna_len
        if args.max_dna_len > 0
        else 128
    )
    export_dummy_text_len = (
        ds_max_text_len
        if "ds_max_text_len" in locals() and ds_max_text_len is not None
        else args.max_text_len
    ...
    dummy_dna_lengths_export = torch.tensor(
        [export_dummy_dna_len // 2], dtype=torch.long
    ).cpu()  # Lengths on CPU for ONNX export
    dummy_text_tokens_export = torch.randint(
        0,
        text_tokenizer.get_vocab_size(),
        (1, export_dummy_text_len),
    ...
    if args.export_onnx:
        if not onnx:
            ...
            logger.info(
                f"Exporting model encoders to ONNX format (using model: {descriptor_for_final_eval})..."
            )
            onnx_export_start_time = time.time()
            dna_encoder_onnx_path = os.path.join(
                args.output_dir, "dna_encoder.onnx"
            )
            try:
                torch.onnx.export(
                    model.dna_encoder,
                ...
                logger.info(
                    f"DNA encoder exported to ONNX: {dna_encoder_onnx_path}"
                )
                onnx_export_paths["dna_encoder"] = dna_encoder_onnx_path
            except Exception as e_onnx_dna:
                logger.error(
            ...
            text_encoder_onnx_path = os.path.join(
            ...
            try:
                ...
                logger.info(
                    f"Text encoder exported to ONNX: {text_encoder_onnx_path}"
                )
                onnx_export_paths["text_encoder"] = text_encoder_onnx_path
            except Exception as e_onnx_text:
                logger.error(
            ...
            with open(
                os.path.join(args.output_dir, "model_web_config.json"), "w"
            ) as f:
                json.dump(web_config, f, indent=2)
            logger.info(
                "Model configuration for web app saved to model_web_config.json"
            )
    ...
    if args.export_tfjs:
        if not args.export_onnx:
            logger.warning(
                "TensorFlow.js export requested, but --export_onnx was not specified. Skipping TF.js export."
            )
        elif not onnx_export_paths:  # If ONNX export failed or was skipped
            logger.warning(
                "ONNX export seems to have failed or was skipped. Cannot proceed with TensorFlow.js export."
            )
        else:
            logger.info(
                f"Exporting ONNX models to TensorFlow.js format (using model: {descriptor_for_final_eval})..."
            )
            if "dna_encoder" in onnx_export_paths:
                tfjs_dna_output_dir = os.path.join(
                    args.output_dir, "tfjs_dna_encoder"
                )
                export_onnx_to_tfjs(
                    onnx_export_paths["dna_encoder"], tfjs_dna_output_dir
                )
            if "text_encoder" in onnx_export_paths:
                tfjs_text_output_dir = os.path.join(
                    args.output_dir, "tfjs_text_encoder"
                )
                export_onnx_to_tfjs(
                    onnx_export_paths["text_encoder"], tfjs_text_output_dir
                )

    logger.info(
        f"Final evaluation and export operations finished in {time.time() - final_ops_start_time:.2f}s."
    )
    ...
    if (
        ...
        chkpt_for_ds_summary = torch.load(
        ...
        if "dataset_summary" in chkpt_for_ds_summary:
            ...
            dataset_summary = {
                "info": "Dataset summary not available from checkpoint or current run."
            }
    elif (
        not dataset_summary
    ):  # Fallback if no input file and no resume with summary
        dataset_summary = {
            "info": "Dataset summary not generated (e.g., no input data processed)."
        }
    ...
    if args.run_dummy_example:
        logger.info(
            f"Dummy example run completed. Dummy input was: {args.input_file}. Dummy outputs are in: {args.output_dir}."
        )
        logger.info(
            f"To clean up, you can remove the base directory used for dummy data: {os.path.dirname(args.input_file)}"
        )

    logger.info(
        f"All processing finished in {time.time() - main_start_time:.2f}s. Main outputs are in: {args.output_dir}"
    )
...
class TestDNAClipComponents(unittest.TestCase):
    def setUp(self):
        ...
        self.kmer_dna_vocab_dict = {
            "ATG": 2,
            "CGC": 3,
            "TAA": 4,
        }  # User-provided part
        self.kmer_dna_tokenizer = KmerDNATokenizer(
            k=self.kmer_dna_k, vocab=self.kmer_dna_vocab_dict
        )
        ...
        self.dummy_fasta_content_expanded = (
            """>s1|species=H|biotype=G\nAAAAACCC\n>s2 description\nGGGGXT\n"""
        )
        self.test_fasta_path = os.path.join(self.temp_dir, "test_data.fasta")
        with open(self.test_fasta_path, "w", encoding="utf-8") as f:
            f.write(self.dummy_fasta_content_expanded)
        ...
        self.test_ref_fasta_path = os.path.join(
            self.temp_dir, "test_ref.fasta"
        )
        with open(self.test_ref_fasta_path, "w", encoding="utf-8") as f:
            f.write(self.dummy_ref_fasta_content)
        ...
        with open(self.test_bed_path, "w", encoding="utf-8") as f:
            f.write(self.dummy_bed_content)
...
"""
        ...
        with open(self.test_gff_path, "w", encoding="utf-8") as f:
            f.write(self.dummy_gff_content)
    ...
    def test_kmer_dna_tokenizer(self):
        self.assertEqual(
            self.kmer_dna_tokenizer.tokenize("ATGCGC"),
            ["ATG", "TGC", "GCG", "CGC"],
        )  # TGC, GCG will be UNK
        ...
        self.assertEqual(
            self.kmer_dna_tokenizer.tokenize("ATGxCGC"), ["ATG", "CGC"]
        )
        ...
        self.assertEqual(
            ids,
            [
                self.kmer_dna_tokenizer.vocab["ATG"],
        ...
        self.assertEqual(
            length, 4
        )  # Effective length is number of actual kmers before padding

        ids_trunc, length_trunc = self.kmer_dna_tokenizer.encode(
            "ATGCGCTAA", max_length=2
        )  # Kmers: ATG, TGC, GCG, CGC, GCT, CTA, TAA
        ...
        ids_short, length_short = self.kmer_dna_tokenizer.encode(
            "AT", max_length=3
        )  # "AT" -> [UNK_TOKEN]
    ...
    def test_char_dna_tokenizer(self):
        self.assertEqual(
            self.char_dna_tokenizer.tokenize("ATGC"), ["A", "T", "G", "C"]
        )
        self.assertEqual(
            self.char_dna_tokenizer.tokenize("ATGx C"), ["A", "T", "G", "C"]
        )  # x and space are filtered
        ...
        expected_ids = [
            DEFAULT_CHAR_DNA_VOCAB["A"],
            DEFAULT_CHAR_DNA_VOCAB["T"],
            DEFAULT_CHAR_DNA_VOCAB["G"],
            DEFAULT_CHAR_DNA_VOCAB["N"],
            self.char_dna_tokenizer.pad_token_id,
        ]
        self.assertEqual(ids, expected_ids)
        self.assertEqual(length, 4)

        ids_trunc, length_trunc = self.char_dna_tokenizer.encode(
            "ATGCGT", max_length=3
        )
        self.assertEqual(
            ids_trunc,
    ...
    def test_text_tokenizer(self):
        self.assertEqual(
            self.text_tokenizer.tokenize("Gene is active!"),
            ["gene", "is", "active"],
        )  # ! removed
        self.assertEqual(
            self.text_tokenizer.tokenize("  Multiple   spaces  "),
            ["multiple", "spaces"],
        )
        self.assertEqual(self.text_tokenizer.tokenize(""), [UNK_TOKEN])

        ids, length = self.text_tokenizer.encode(
            "Gene is active", max_length=4
        )
        ...
        expected_ids = [
            self.text_tokenizer.vocab["gene"],
            self.text_tokenizer.vocab["is"],
            self.text_tokenizer.vocab["active"],
    ...
    def test_vocab_building(self):
        dna_texts = [
            ...
            "CGCGCG",
            "ATATAT",
        ]  # k=3: ATG,TGC,GCG; CGC,GCG,CGC; ATA,TAT,ATA
        kmer_tok_built = build_vocab_from_data(
            dna_texts, KmerDNATokenizer, min_freq=1, tokenizer_args={"k": 3}
        )
        ...
        char_tok_built = build_vocab_from_data(
            dna_texts, CharDNATokenizer, min_freq=1
        )  # Uses fixed default vocab
        self.assertEqual(
            len(char_tok_built.vocab), len(DEFAULT_CHAR_DNA_VOCAB)
        )
        self.assertIn("A", char_tok_built.vocab)

    def test_dataset_item_creation(self):
        seqs = ["ATGCGTAGC", "TTT"]
        anns = ["active gene", "inactive"]
        metadata = [{"id": "s1"}, {"id": "s2"}]
        # Kmer tokenizer with k=3. "ATGCGTAGC" -> ATG,TGC,GCG,CGT,GTA,TAG,AGC (7 kmers)
        dataset_kmer = DNADataset(
            seqs,
            anns,
            self.kmer_dna_tokenizer,
            self.text_tokenizer,
            max_dna_len=5,
            max_text_len=3,
            metadata=metadata,
        )
        item0_kmer = dataset_kmer[
            0
        ]  # "ATGCGTAGC" (7 kmers) -> truncated to 5 kmers. "active gene" (2 tokens)
        self.assertEqual(item0_kmer["dna_tokens"].shape, torch.Size([5]))
        self.assertEqual(item0_kmer["dna_lengths"].item(), 5)
        self.assertEqual(item0_kmer["text_tokens"].shape, torch.Size([3]))
        self.assertEqual(item0_kmer["text_lengths"].item(), 2)
        self.assertTrue(
            item0_kmer["text_tokens"][-1] == self.text_tokenizer.pad_token_id
        )

        # Char tokenizer. "TTT" (3 chars). "inactive" (1 token)
        dataset_char = DNADataset(
            seqs,
            anns,
            self.char_dna_tokenizer,
            self.text_tokenizer,
            max_dna_len=10,
            max_text_len=4,
            metadata=metadata,
        ...
        self.assertEqual(item1_char["dna_tokens"].shape, torch.Size([10]))
        self.assertEqual(item1_char["dna_lengths"].item(), 3)
        self.assertTrue(
            all(
                t == self.char_dna_tokenizer.pad_token_id
                for t in item1_char["dna_tokens"][3:].tolist()
            )
        )
        self.assertEqual(item1_char["text_tokens"].shape, torch.Size([4]))
        self.assertEqual(item1_char["text_lengths"].item(), 1)

    def test_custom_collate_fn(self):
        batch_items_for_collate = [
            {
                "dna_tokens": torch.tensor([2, 3, 4]),
                "dna_lengths": torch.tensor(3),
                "text_tokens": torch.tensor([2, 3]),
                "text_lengths": torch.tensor(2),
                "metadata": {},
            },
            {
                "dna_tokens": torch.tensor([5]),
                "dna_lengths": torch.tensor(1),
                "text_tokens": torch.tensor([4, 5, 6, 7]),
        ...
        self.assertTrue(
            torch.equal(collated["dna_lengths"], torch.tensor([3, 1]))
        )
        self.assertTrue(
            torch.equal(collated["text_lengths"], torch.tensor([2, 4]))
        )
    ...
    def test_dnaclipmodel_forward(self):
        ...
        model = DNAClipModel(
            ...
            text_pad_idx=self.text_tokenizer.pad_token_id,
        ).to(self.device)

        bs = 2
        max_dna_len, max_text_len = 10, 8
        dna_toks = torch.randint(
            0, dna_vocab_sz, (bs, max_dna_len), dtype=torch.long
        ).to(self.device)
        dna_lens = torch.tensor(
            [max_dna_len, max_dna_len // 2], dtype=torch.long
        )  # CPU
        text_toks = torch.randint(
            0, text_vocab_sz, (bs, max_text_len), dtype=torch.long
        ).to(self.device)
        text_lens = torch.tensor(
            [max_text_len // 2, max_text_len], dtype=torch.long
        )  # CPU

        logits_dna, logits_text, temp = model(
            dna_toks, dna_lens.cpu(), text_toks, text_lens.cpu()
        )
    ...
    def test_contrastive_loss(self):
        bs = 4
        sim_good = (
            torch.eye(bs, device=self.device) * 10
    ...
    def test_calculate_mrr(self):
        ...
        sim_matrix_worst = np.array(
            [[0.1, 0.8, 0.9], [0.8, 0.1, 0.7], [0.9, 0.7, 0.1]]
        )  # Correct target is rank 3
        ...
        sim_matrix_ranks_varied = np.array(
            [[0.9, 0.1, 0.0], [0.1, 0.0, 0.9], [0.0, 0.9, 0.1]]
        )
    ...
    def test_parse_fasta(self):
        ...
        self.assertEqual(
            parsed[1][2].get("description_freeform", "").strip(), "description"
        )
        self.assertEqual(
            parsed[1][2]["species"], "unknown_species_fasta_header"
        )  # Default
    ...
    def test_parse_bed(self):
        ...
        self.assertEqual(
            parsed[0][2]["biotype"], "coding_bed"
        )  # From BED attributes
        self.assertEqual(
            parsed[0][2]["species"], "TestSpecies"
        )  # From ref FASTA header for chrTest1
        ...
        self.assertEqual(
            parsed[1][1], "NNNAT"
        )  # Sequence from chrTest1[10:15]
        self.assertEqual(
            parsed[1][2]["biotype"], "RefContig"
        )  # Default from ref FASTA header for chrTest1
        self.assertEqual(parsed[1][2]["species"], "TestSpecies")
        self.assertIn(
            "feat2_coords_are_valid from TestSpecies chromosome chrTest1:10-15",
            parsed[1][3],
        )
    ...
    def test_parse_gff3(self):
        ...
        parsed = parse_gff3(
            self.test_gff_path, self.test_ref_fasta_path, target_types
        )
        ...
        self.assertEqual(
            parsed[0][2]["species"], "TestSpecies"
        )  # From ref FASTA for chrTest1
        self.assertEqual(parsed[0][2]["gff_feature_type"], "gene")
        self.assertIn(
            "protein_coding_gff. TestGene1. from TestSpecies chromosome chrTest1:1-6(+).",
            parsed[0][3],
        )
        ...
        self.assertEqual(
            parsed[1][2]["biotype"], "mRNA"
        )  # Defaulted to feature type as no biotype attr
        self.assertEqual(parsed[1][2]["species"], "TestSpecies")
        self.assertEqual(parsed[1][2]["gff_feature_type"], "mRNA")
        self.assertIn(
            "mRNA. rna1. from TestSpecies chromosome chrTest1:10-15(-). Description: Test RNA on minus strand.",
            parsed[1][3],
        )

    @unittest.skipIf(
        onnx is None, "ONNX library not installed, skipping ONNX export test."
    )
    def test_onnx_export_conceptual(self):
        model = DNAClipModel(20, 20, 16, 24, 1).to(self.device)
        model.eval()
        dummy_tokens = torch.randint(0, 20, (1, 10), dtype=torch.long).to(
            self.device
        )
        ...
        try:
            ...
            self.fail(f"ONNX export for DNA encoder failed: {e}")

    @unittest.skipIf(
        any(
            lib is None
            for lib in [tensorflow, onnx, onnx_tf_prepare, tensorflowjs]
        ),
    ...
    def test_tfjs_export_conceptual(self):
        ...
        if onnx:  # Guard for type hinting if onnx is None
            ...
            X = helper.make_tensor_value_info(
                "X", TensorProto.FLOAT, [None, 3]
            )  # Dynamic batch, 3 features
            Y = helper.make_tensor_value_info(
                "Y", TensorProto.FLOAT, [None, 3]
            )
            node_def = helper.make_node("Identity", ["X"], ["Y"])
            graph_def = helper.make_graph([node_def], "dummy-model", [X], [Y])
            model_def = helper.make_model(
                graph_def, producer_name="unittest-tfjs"
            )
        ...
        try:
            ...
            with unittest.mock.patch("subprocess.run") as mock_subproc_run:
                ...
                called_args = mock_subproc_run.call_args[0][
                    0
                ]  # First arg of first call
                self.assertIn(
                    tfjs_output_path, called_args
                )  # Check if output path was in command
