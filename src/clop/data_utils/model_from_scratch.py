...
def sequence_to_tensor(sequence, nucleotide_map):
    return torch.tensor([nucleotide_map[nuc] for nuc in sequence], dtype=torch.long)

def annotation_to_tensor(annotation, tokenizer):
    return tokenizer.encode(annotation, return_tensors="pt", add_special_tokens=True).squeeze(0)

# Enhanced Sequence Encoder
class DNASEquenceEncoder(nn.Module):
    def __init__(self, embedding_dim):
        super(DNASEquenceEncoder, self).__init__()
        self.embedding = nn.Embedding(5, embedding_dim)  # 4 nucleotides + 1 for padding
        ...
        self.bi_lstm = nn.LSTM(128, 128, num_layers=2, bidirectional=True, batch_first=True)
        self.fc = nn.Linear(256, embedding_dim)  # 256 = 128 * 2 for bidirectional

    def forward(self, x, lengths):
        x = self.embedding(x).permute(0, 2, 1)  # [batch, embedding_dim, sequence_length]
        x = self.conv_layers(x).permute(0, 2, 1)  # [batch, sequence_length, channels]
...
class FastaDataset(Dataset):
    def __init__(self, file_path, nucleotide_map, tokenizer):
        self.sequences, self.annotations, self.seq_lengths, self.attention_masks = self.process_fasta(file_path, nucleotide_map, tokenizer)
    ...
    def process_fasta(self, file_path, nucleotide_map, tokenizer):
        ...
        with open_func(file_path, 'rt') as f:
            for record in SeqIO.parse(f, "fasta"):
                ...
                encoded_annotation = annotation_to_tensor(annotation, tokenizer)
                annotations.append(encoded_annotation)
                seq_lengths.append(len(sequence))
                attention_masks.append(torch.tensor([1] * encoded_annotation.size(0), dtype=torch.long))

        return pad_sequence(sequences, batch_first=True), pad_sequence(annotations, batch_first=True), torch.tensor(seq_lengths), pad_sequence(attention_masks, batch_first=True)

    def __len__(self):
        return len(self.sequences)

    def __getitem__(self, idx):
        return self.sequences[idx], self.annotations[idx], self.seq_lengths[idx], self.attention_masks[idx]
...
# Example Usage
model = DNA_CLIP()
fasta_file_path = 'path_to_fasta_file.fasta.gz'  # Supports .zip, .gz, .bz2, or uncompressed
...
for sequences, annotations, seq_lengths, attention_masks in data_loader:
    optimizer.zero_grad()
    sequence_emb, annotation_emb = model(sequences, annotations, seq_lengths, attention_masks)
    
    # Compute loss, for example, using Mean Squared Error (modify according to your needs)
    loss = loss_function(sequence_emb, annotation_emb)  # Assuming a simplistic approach for demonstration
