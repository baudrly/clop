...
def calculate_voxel_density(image, scale, max_intensity_level):
    ...
    for x in range(0, image.shape[0], scale):
        for y in range(0, image.shape[1], scale):
            if is_uniform:
                voxel_set.add((x // scale, y // scale, int(scaled_intensity[0, 0])))
            else:
                block = scaled_intensity[x:x+scale, y:y+scale]
                if block.size:
                    min_intensity, max_intensity = np.floor(block.min()), np.ceil(block.max())
                    for z in range(int(min_intensity), int(max_intensity) + 1):
                        voxel_set.add((x // scale, y // scale, z))
    return len(voxel_set)

def compute_fractal_dimension(image, max_box_size=32, min_box_size=2, step=2, max_intensity_level=32):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        futures = [executor.submit(calculate_voxel_density, normalized_image, scale, max_intensity_level) for scale in scales]
...
def test_uniform_array():
    ...
    assert np.isclose(fd, 0, atol=0.1), "Fractal dimension of a uniform array should be close to 0."
...
def test_sierpinski_triangle():
    ...
    assert np.isclose(fd, theoretical_fd, atol=0.2), f"Expected fractal dimension close to {theoretical_fd}, got {fd}"
